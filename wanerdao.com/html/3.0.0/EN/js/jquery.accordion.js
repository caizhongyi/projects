            ...
            $(this).off(clickEvent + '.accordion').on(clickEvent + '.accordion', options.heading + ',[data-target]', function () {
                    ...
                    if (!$panel.is(':animated')) {
                        if (me.hasClass('horizontal') || me.hasClass('accordion-horizontal')) {
                            if (me.hasClass('dockbar')) {
                                ...
                                $panel.stop(true).animate({ width: 'toggle' } , options.duration , options.easing ,function(){ me.toggleClass('active');});

                        }
                        else {
                            $panel.stop(true).slideToggle(options.duration , options.easing ,function(){ me.toggleClass('active'); });
                        }
                ...
                if (!$panel.is(':animated')) {
                    ...
                    if (ishorizontal) {
                        ...
                        options.collapse ? $item.find(options.panel).slideDown(options.duration , options.easing ,function(){ $item.addClass('active')}) : $item.find(options.panel).slideToggle(options.duration , options.easing ,function(){ $item.toggleClass('active')});
                    }
                    me.trigger('change', { item: $item });
                    me.data('temp', $item);
                }
