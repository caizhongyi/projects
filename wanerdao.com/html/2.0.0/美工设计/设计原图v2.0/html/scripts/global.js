...
$.fn.placeholder = function(){
    ...
    $(this).each(function(){//请勿对输入框和文本区域以外的标签使用inputdefault属性
        if($(this).attr('init-placeholder')){
            return ;
        }
        $(this).attr('init-placeholder',true);
        var color = $(this).css('color');
        if(color == '#666666' || color == 'rgb(102, 102, 102)'){
            color = '#999';
        };
        $(this).wrap('<span style="display: inline-block;position: relative;"></span>').val('');
        var _this = this;
...
function wd_init(){
    ...
    $('[trunk8]').each(function(i, n){
     ...
     $('input[button-loading]').click(function(){
         ...
         if($(this).attr('button-loading-class'))
