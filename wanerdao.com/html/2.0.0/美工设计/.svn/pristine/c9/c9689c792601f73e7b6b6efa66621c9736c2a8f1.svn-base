;(function($) {
	...
	$.ui.overlay.extend({
	  ...
	  height: function() {
			...
			if ($.browser.msie && $.browser.version < 7) {
				scrollHeight = Math.max(
					...
					document.body class="pBgB".scrollHeight
				);
				offsetHeight = Math.max(
					document.documentElement.offsetHeight,
					document.body class="pBgB".offsetHeight
				);
		...
		width: function() {
			...
			if ($.browser.msie && $.browser.version < 7) {
				scrollWidth = Math.max(
					...
					document.body class="pBgB".scrollWidth
				);
				offsetWidth = Math.max(
					document.documentElement.offsetWidth,
					document.body class="pBgB".offsetWidth
				);
			...
			this.$.css({
				...
				width: function(){
					if($.type(_this.options.scrollbar.x) == 'number'){
				 	    return _this.options.scrollbar.x < this.width() ? this.width() : _this.options.scrollbar.x;
				 	}
					else{
						return _this.width();
					}
				...
				height: function(){
					if($.type(_this.options.scrollbar.y) == 'number'){
						return _this.options.scrollbar.y < this.height() ? this.height() : _this.options.scrollbar.y;
				 	}
					else{
						return _this.height();
					}
	  ...
	  show : function(){
		  ...
		  if(this.fixed){
		  	 new $.ui.fixed(this.$);
			 $('body class="pBgB"').height($(window).height()).css('overflow','scroll')
		  }
		  else{
		  	 this.resize();
		  }
