...
function pop(options) {
    ...
    infopop = {
        ...
        show: function (opts) {
            ...
            html += '       <a href="javascript:;" class="button button1" id="btnpopOK">' +  + '</a>';
            if (opts.typename === 'confirm') {
                html = '<a href="javascript:;"  class="button button1-disable" >' +  + '</a>';
            }
            html += '     </div>';
            html += '</div>';
            html += '</div>';
