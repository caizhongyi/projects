...
;(function($) {
	...
	$.ui.accordion.extend({
	   ...
	   show    : function(index , callback){
		   ...
		   if(this.options.animate){
			   this.animate($obj.show() , { height : this.maxheight() , opacity : 1 }, function(){})
		   }
		   else{
		   	   $obj.show();
		   }
	   ...
	   hide    : function(index , callback){
		   ...
		   if(this.options.animate){
			   this.animate($obj , { height : this.minheight() , opacity : opacity } , function(){});
		   }
		   else{
		   	   $obj.hide();
		   }
	...
	$.ui.accordions = function(selector,options){
		var arr = [];
		$(selector).each(function(i,n){
			arr.push(new $.ui.accordion(n,options));
		})
		return arr;
	}
})(jQuery);﻿