...
namespace WanerDao.Indexing.Database
{
    ...
    public class CommonHelper
    {
        ...
        public static List<SQLTable> GetSQLConfigList(string xmlPath)
        {
            string path = xmlPath;
            List<SQLTable> resultList = new List<SQLTable>();
            string dir = AppDomain.CurrentDomain.BaseDirectory;//web程序的bin路径
            XElement xEle = XDocument.Load(dir + path).Root;
            var query = from x in xEle.Descendants("add")
                        select new
                        {
                            tableName = x.Attribute("tableName").Value,
                            indexColumn = x.Attribute("indexColumn").Value,
                            tableCategory = x.Attribute("tableCategory").Value,
                            otherColumn = x.Attribute("otherColumn").Value
                        };

            foreach (var item in query)
            {
                SQLTable sqlTab = new SQLTable();
                sqlTab.indexColumn = item.indexColumn;
                sqlTab.otherColumn = item.otherColumn;
                sqlTab.tableCategory = item.tableCategory;
                sqlTab.tableName = item.tableName;
                resultList.Add(sqlTab);
            }
            return resultList;
        }
