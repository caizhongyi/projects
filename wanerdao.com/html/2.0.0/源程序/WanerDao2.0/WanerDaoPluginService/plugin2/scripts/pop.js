$(function () {
    ...
    /*弹窗拖动*/
    $(".popDiv .popTitle").mousedown(function (event) {
        var left, top, x, y, offset = 0;
        x = event.pageX;
        y = event.pageY;
        if ($(this).parent().parent().get(0).offsetTop + $(this).parent().parent().height() > $(window).height()) {
            offset = 38;
        }
        top = parseInt($(this).parent().parent().get(0).offsetTop + $(this).parent().parent().height() / 2 - offset);
        left = parseInt($(this).parent().parent().get(0).offsetLeft + $(this).parent().parent().width() / 2);
        $(this).parent().parent().bind("mousemove", function (event) {
            var iLeft = event.pageX - x + left;
            var iTop = event.pageY - y + top;
...
function closeWin(content_) {
    //	$(content_).hide();
    //	$("#popBg").hide();
    var mainDomain = baseUrl + "crossdomain/crossdomain.htm#close";
    $(document).find("body").append("<iframe style='display: none;' src='" + mainDomain + "'></iframe>");
}
