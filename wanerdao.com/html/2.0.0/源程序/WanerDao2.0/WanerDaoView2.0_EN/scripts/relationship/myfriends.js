...
function initData() {
    ...
    $('.layer_btn').floatbox('click', { position: 'b', box: '.layer', move: 'false', offset: { x: 0} })

    $(function () {

        $(".layer_btn").toggle(function () {
            $('#selectclassUL').empty();
//            $(this).parent().parent().css({ position: "relative" });
//            $(this).parent().parent().find(".layer").css({ display: "block" });
            //$(this).parent().parent().find(".layer").css({ position: "absolute", 'top': $(obj).offset().top + 20, 'left': $(obj).offset().left });
        }, function () {
            $('#selectclassUL').empty();
          //  $(this).parent().parent().find(".layer").css({ display: "none" });
        })
