    ...
    $.fn.AutoCompelteSimple = function (options) {
        ...
        if (options.displayValueField && options.displayTextField && options.operType && options.callback) {
            ...
            $(this).bind('keyup', function (e) {
                ...
                jMain.css({ 'position': 'absolute', 'width': width + 'px', 'top': top + 'px', 'left': +left + 'px', 'z-index': '5999', 'background': '#fff' });
                ...
                if ($(this).val()) {
                    if (e.keyCode == '38') {
                        ...
                        if (options.showTip) {
                            jTip.html('<img src="/images/loading.gif" style="float:right; width: 16px; height: 16px;"/>');
                        }
                        getData($(this).val());
                    }
                }
            ...
            var renderData = function (data) {
                ...
                if (typeof data != 'undefined' && data.rows && data.rows.length > 0) {
                    ...
                    ul.find('li').css({ 'height': defaults.itemHieght + 'px', 'cursor': 'pointer' });
