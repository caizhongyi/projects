(function ($) {
    var typeName = ['Activity', 'Group', 'Blog', 'Info'];
    var selectName = ['Activity', 'Group', 'Blog', 'Info'];
    ...
    $.fn.ShareTools = function (options) {
        ...
        function showUI(url, container, opertype, type, callback) {
            container.html('<img src="/images/loading.gif" style=" width: 17px; height: 17px;"/>');
            ajaxData(url, {
                opertype: opertype,
                pagecurrent: 1,
                pageSize: 10000
                ...
                container.html('');
                container.append('<input type="checkbox" class="sharechk inp04" /> <label>Share to ' + typeName[type] + '</label> ');
                var list = jQuery('<select></select>').appendTo(container);
                list.append(format('<option value="">Choose{0}</option>', selectName[type]));
                ...
                $('select').chosen();
                container.append(' ');
                var input = jQuery('<input type="text" cid="" class="text" />').appendTo(container);
