...
function successActivityInforFunc(data) {
    ...
    strContent += ("<p class=\"plan_info\"><span>活动描述：</span>" + data.data.description + "</p>");
    ...
    if (data.data.ActivityPlan != null && data.data.ActivityPlan != "") {
        $.each(data.data.ActivityPlan, function (i, msg) {
            strPlanContent += (msg.start_date + msg.plan_content);
        });
    }
    ...
    strContent += ("<p class=\"join_event_plan\"><a href=\"###\"  onclick=\"displaysignup()\" class=\"join-button button\">参加活动</a>&nbsp;&nbsp;<a href=\"###\"  onclick='copyUrl();' class=\"gay-button_110x28 button\">复制活动链接</a></p>");
...
var stateid = "";
var cityid = "";
$("#txtAcArea").click(function () {
    new wanerdaoArea({ comopts: { elementid: "#txtAcArea", callback: function (data) {
        countryid = data.cid;
        stateid = data.sid;
        cityid = data.cityid;
        address = $("#txtAcArea").val();
    }
    }
    });
});
