...
//            }
            ...
            $.each(data.rows, function (i, item) {
                ...
                $li.find(".cancelFollow").click(function () {
                    $.ajax({
                        ...
                        dataType: "json",
                        cache: false,
                        data: "{opertype:'cancelpersonalactivityfollow',attention_id:'" + item.follow_id + "'}",
                        error: function () {
                        },
                        success: function (data) {
                            if (data.result) {
    ...
    if (is_self) {
        ...
        function addVedio() {
            ...
            if (getStringLength(videoUrl) > 200) {
            ...
            ajaxCommon("/sharevideo_home.axd",
                            "{ opertype: 'sharevideo',videourl:'" + videoUrl + "',videodesc:'" + videoDesc + "'}",
                            function (data) {
                                if (data.result) {
                                    hint(wanerdaoLangTip("share_00001"), $("#hint"));
                                    $("#vedioUrl").val("http://");
