(function ($) {
    ...
    $.fn.fgAutoCompelte = function (options, otrigger) {
        ...
        if (options.displayValueField && options.displayTextField && options.getData && options.callback) {
            ...
            var jMain = jQuery('<div class="fmain"></div>').appendTo($(this).parent()).css({ 'position': 'absolute', 'width': width + 'px', 'top': top + 'px', 'left': +left + 'px' });
            var jTip = jQuery('<div class="ftip"></div>').appendTo(jMain).css({ 'height': '20px' });
