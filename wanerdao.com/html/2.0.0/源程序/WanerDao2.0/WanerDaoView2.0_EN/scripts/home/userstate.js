...
*/
function visitSelf() {
    $('#stateTab').append('<a href="javascript:;" class="active">New feeds</a> <a href="javascript:;">Comment</a> <a href="javascript:;">Group</a> <a href="javascript:;">Activity</a>');
...
*/
function visitOther() {
    $($('.main').children().get(0)).before('<div class="black10"></div><div class="mes_com_box_Tab"><a href="javascript:;" id="tab_zl">Profile</a><a href="javascript:;" id="tab_rz">Diary</a><a href="javascript:;" id="tab_xc">Album</a><a href="javascript:;" id="tab_sp">Video</a></div>');
    bindPTab(uid);
    $('#tab_home').addClass('active');
    $('#stateTab').append('<a href="javascript:;" class="active">New comment</a>');
    Init('message');
}
...
function callback(data) {
    if (data.result) {
        ...
        hasMore = false;
        $('.loading').html('<div style=" line-height:50px; color: #999;">No more</div>');
        $('.moreStateBar').hide();
    }
}
...
function options_reply($obj, opertype, category) {
    ...
    var tool_reply = jQuery('<a href="javascript:;" class="replay">Reply(0)</a>').appendTo($obj.find('.comments-options'));
...
function options_setBtn($obj, ignore, ignoreAll, shield, focus, trash) {
    ...
    //忽略该更新
    if (ignore) {
        var li_IgnoreThis = jQuery('<li>·Ignore</li>').appendTo(tool_setBtn.find('ul'));
    ...
    //忽略该人的全部更新
    if (ignoreAll) {
        var li_IgnoreAll = jQuery('<li>·Ignore all of the one</li>').appendTo(tool_setBtn.find('ul'));
    ...
    //屏蔽该人的更新
    if (shield) {
        var li_Shield = jQuery('<li>·Shield</li>').appendTo(tool_setBtn.find('ul'));
    ...
    //关注该人
    if (focus) {
        var li_Focus = jQuery('<li>·Focus</li>').appendTo(tool_setBtn.find('ul'));
    ...
    //垃圾信息举报
    if (trash) {
        var li_Trash = jQuery('<li>·Trash</li>').appendTo(tool_setBtn.find('ul'));
        li_Trash.click(function () {
            ajaxfunc('shield_home.axd', "{opertype:'dustinforeport',source_type_id:'" + $obj.find('.stateId').val() + "',target_user_id:'" + uid + "'}", null, function (data) {
                if (data.result) {
                    new pop({ typename: 'success', msginfo: 'Report success!' });
                }
            });
        }).css('cursor', 'pointer');
    }
