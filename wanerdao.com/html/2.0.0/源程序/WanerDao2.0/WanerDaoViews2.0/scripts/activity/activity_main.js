...
$("i[action=follow]").live("click", function () {
    var obj = $(this);
    var objtype = obj.attr("type");
    if (objtype == "1") {
        followactivitycategory('followactivitymodule', obj.attr("id"), objtype);
    }
    else if (objtype == "0") {
        followactivitycategory('cancelactivitymodulefollow', obj.attr("id"), objtype);
    }
});
function followactivitycategory(opertype, categoryid, objtype) {
    ajaxfunc("activitycategory_follow.axd", "{opertype:'" + opertype + "',moduleId:'" + categoryid + "'}", function (data) {
        new pop({ titleid: 'common_00022', typename: 'error', msginfo: data.msg });
    }, function (data) {
        if (data.result == true) {
            var o = $("#" + categoryid).parent().parent().find('i:last-child span:first');
            if (objtype == "1") {
                $("#" + categoryid).removeClass("rHeart emptyH").addClass("rHeart fullH");
                o.html("<em class=\"icon_1\"></em>" + (parseInt(o.text()) + 1));
            }
            else if (objtype == "0") {
                $("#" + categoryid).removeClass("rHeart fullH").addClass("rHeart emptyH");
                o.html("<em class=\"icon_1\"></em>" + (parseInt(o.text()) - 1));
            }
        }
        else {
        ...
        $.each(data.rows, function (i, msg) {
            ...
            if (data.rows.length > 7) {
                if (data.rows.length - 1 === i) {
                    v_content += ("<li class='last'>");
                }
                else if (data.rows.length - 2 === i) {
                    v_content += ("<li class='last'>");
                }
                else if (data.rows.length - 3 === i) {
                    v_content += ("<li class='last'>");
                }
                else {
                    v_content += ("<li>");
            ...
            if (msg.isfollow == "1") {
                v_content += ("<i class=\"rHeart fullH\" action='follow' type='0' id='" + msg.category_id + "'></i>");
            }
            else {
                v_content += ("<i class=\"rHeart emptyH\" action='follow' type='1' id='" + msg.category_id + "'></i>");
            }
            v_content += ("</i>");
        ...
        */
        function _formatAddress(data) {
            ...
            switch (data.country_id) {
                case country["cn"]: //中国地址。格式为国家,省,城市,地址
                    partAddr = data.country_name + "," + data.state_name + "," + data.city_name;
                    detailAddr = partAddr + "," + data.address;
                    break;
                case country["us"]: //美国地址。格式为地址,城市,州,国家
                    partAddr = data.city_name + "," + data.state_name + "," + data.country_name;
        ...
        function _searchActivity(callBack) {
            ...
            param.CurrLatLng = { Lat: lat, Lng: lng };

            var dist = $("#disToHome").val();
            param.FromHomeDistance = (dist == -2 ? 1000000 : dist); //单位是公里,默认15公里
        ...
        //加载地图
        function _loadMap(options, markers) {
            ...
            if (markers)
                t.markers = markers;
            var zoom = opt.radius ? 12 - opt.radius / 10000 : 11;
            //加载google map地图
            $('#map').gmap3(
            { action: 'addCircle',
                circle: {
            ...
            { action: 'addMarkers',
                ...
                marker: {
                    options: {
                        draggable: false,
                        icon: new google.maps.MarkerImage("/images/icon_location.gif")
                    }, events: {
    ...
    function MapPanel() {
        ...
        function render() {
            ...
            $("#search").click(function () {
                if (getActFlag) {
                    getActFlag = false;
                    _getCurrentLocation({}, function (opt) {
                        _searchActivity(function (data) {
                            ...
                            if (data.result) {
                                ...
                                $.each(data.rows, function (i, item) {
                                    ...
                                    content = wanerdaoLangTip('active_00046') + "：<a href='/activity/activity_index.html?id=" + item.activityId + "' target='_blank'>" + item.activityName + "</a><br/>" + wanerdaoLangTip('active_00047') + "：" + item.activityAddress;
