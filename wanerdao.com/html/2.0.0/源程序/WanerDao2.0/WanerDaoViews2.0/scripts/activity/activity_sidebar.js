...
//        sidebarpage: true,
        ...
        callback: pageHtmlContent,
        navcls: 'sidebox-pager  f_right', //上下页导航的样式
        pagermore: false,
        ajax: {
            url: 'pop_common.axd',
            param: {
...
function getContent(data, totalCount) {
    ...
    $.each(data.rows, function (i, msg) {
        ...
        strContent += subPoints(msg.description, 100);
        strContent += "<p><span>" + msg.join_member_nbr + "/" + msg.max_nbr + "人&nbsp;&nbsp;" + getMonthAndDate(begindate) + "&nbsp;-" + getMonthAndDate(enddate) + "</span>" + vSingup + "</p>";
        strContent += "</li>";
    });
    return strContent
}
...
//     alert(cityid);
    ...
    var reg = new RegExp(',', "g"); //.replace(/\s/g, "")
    if ($("#txt_activityNames").val() != "关键词请用英文状态的冒号来分隔") activityNames = $("#txt_activityNames").val().replace(reg, ':'); //活动名字串，用“:”分隔 可以缺省默认为空
    else { activityNames = ""; }
    if ($("#txt_categoryNames").val() != "请选择活动分类") catygoryNames = $("#txt_categoryNames").val().replace(reg, ':'); //分类名字串，用“:”分隔 可以缺省默认为空
    else { catygoryNames = ""; }
    if ($("#txt_friendsName").val() != "好友参加的活动") friendsName = $("#txt_friendsName").val().replace(reg, ':'); //朋友名字串，用“:”分隔 可以缺省默认为空
    else { friendsName = ""; }
    if ($("#txt_groupNames").val() != "圈子参加的活动") groupNames = $("#txt_groupNames").val().replace(reg, ':'); //圈子名字串，用“:”分隔 可以缺省默认为空
    else { groupNames = ""; }
    if ($("#txt_sightNames").val() != "景点") sightNames = $("#txt_sightNames").val().replace(reg, ':'); //景点名字串，用“:”分隔 可以缺省默认为空
    else { sightNames = ""; }
    if ($("#txtactivitycountryName").val() == "请选择地区") {
        ...
        address= $("#txtactivitycountryName").val().replace(reg, ':'); 
    }
