...
function submitValidation() {
    if (isf) {
        ...
        vsummary = document.getElementById("txt_summary").value.replace(reg, "<br>");
        ...
        vdescription = document.getElementById("txt_description").value.replace(reg, "<br>");
...
function createGroup() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result) {
                $("#btn_sub").removeClass("blue-radius-btn").addClass("disable-radius-btn").html(wanerdaoLangTip('relationship_00038'));
