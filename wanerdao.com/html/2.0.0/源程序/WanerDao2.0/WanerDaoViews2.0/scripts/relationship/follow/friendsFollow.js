...
function updateDuration(id, isEmail, duration, callBack) {
    $.ajax({
        url: "/updatepersonalfriends_follow.axd",
        type: "POST",
        dataType: "json",
        cache: false,
...
//取消关注
function cancelFollow(friendId, callBack) {
    $.ajax({
        url: "/cancelpersonalfriends_follow.axd",
        type: "POST",
        dataType: "json",
        cache: false,
        data: "{opertype:'cancelpersonalfriendsfollow',friendId:'" + friendId + "'}",
        error: function () {
        },
        success: function (data) {
            callBack(data);
    ...
    function PageRender() {
        ...
        function _render(str) {
            ...
            $(".pagewrap").myPagination({
                ...
                ajax: {
                    url: '/getpersonalfriends_follow.axd', //此处必须填写，分页已没有固定处理工厂
                    param: {
                        pagecurrent: 1,
                        pageSize: 10,
                        friendName: searchStr,
                        opertype: 'getpersonalfriendsfollow'//操作码
                    },
                    info: {
                        first: '首页',
                        last: '尾页',
        ...
        function _renderContent(data, total) {
            ...
            $.each(data.rows, function (i, msg) {
                ...
                $li.append("<div class='fd_b_name'><a href='/personal/personal_info.html?uid=" + msg.attention_id + "&infoType=1' target='_blank'>" + msg.name + "</a> <i class='level'>等级 <b>" + msg.experience + "</b> 级</i></div>");
