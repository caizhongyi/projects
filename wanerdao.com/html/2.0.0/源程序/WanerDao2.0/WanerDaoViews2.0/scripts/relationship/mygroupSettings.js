...
function validationdigest() {
    if (!isDigit($("#txt_digest").val())) {
        $("#txt_digest").val("");
        new pop({ typename: 'error', msginfo: wanerdaoLangTip('relationship_00069') });
    }
}
...
function successret(data) {
    ...
    settingID = data.rows[0].id;
        ...
        if (data.rows[0].is_email_event == "False" && data.rows[0].is_notice_event == "False") {
            ...
            $("#chk_event").attr("checked", "checked");
        }
...
function submitValidation() {
    existgroup(groupid, submitValidationT);

}

function submitValidationT() {
    var validation = true;
    $(".msgLabArr").html("");
...
function saveSetting() {
    existgroup(groupid, saveSettingT);

}

function saveSettingT() {
