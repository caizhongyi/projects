...
//取消关注
function cancelFollow(activityId, callBack) {
    $.ajax({
        url: "/cancelpersonalactivity_follow.axd",
        type: "POST",
        dataType: "json",
        cache: false,
        data: "{opertype:'cancelpersonalactivityfollow',activityId:'" + activityId + "'}",
        error: function () {
        },
        success: function (data) {
            callBack(data);
    ...
    function PageRender() {
        ...
        function _render(str) {
            ...
            $(".pagewrap").myPagination({
                ...
                ajax: {
                    url: '/getpersonalactivity_follow.axd', //此处必须填写，分页已没有固定处理工厂
                    param: {
                        pagecurrent: 1,
                        pageSize: 10,
                        activityTitle: searchStr,
                        opertype: 'getpersonalactivityfollow'//操作码
                    },
                    info: {
                        first: '首页',
                        last: '尾页',
        ...
        function _renderContent(data, total) {
            ...
            $.each(data.rows, function (i, msg) {
                ...
                $li.append("<input type='hidden' class='followId' value='" + msg.id + "'/><h1>[" + msg.category_name + "] <a href='/activity/activity_index.html?id=" + msg.attention_id + "' target='_blank'>" + msg.activity_name + "</a><font color='#cc0000'>（" + msg.join_member_nbr + "人）</font></h1>");
                ...
                $li.find(".follow_list_con").append("<div class='follow_list_jd'><label class='ml12'>活动时间段：</label>" + getLocationDateString(msg.begin_datetime, "MM/dd hh") + " -  " + getLocationDateString(msg.end_datetime, "MM/dd hh") + "<label>活动地点：</label>" + msg.address + "</div>");
