...
function initDataT() {
    ...
    $("#btnaddmanager").click(function () {
        wanerdaoPop({
            comopts: { titleid: 'common_00012', typename: 'group', groupid: groupid, elementid: 'btnaddmanager', callback: setaddManager }
        });
    });

    $("#btnaddfinance").click(function () {
        wanerdaoPop({
            comopts: { titleid: 'common_00012', typename: 'group', groupid: groupid, elementid: 'btnaddfinance', callback: setaddfinance }
        });
    });
...
function setaddManager(data) {
    $.each(data.group, function (i, v) {
        ...
        $("#managerDIV span").each(function () {
            if ($.trim(v.id) == $.trim($(this).attr("lang"))) {
                vi = false;
            }
        });
        if (vi) {
...
function reCategory() {
    if (cateID == "") {
        window.setTimeout(reCategory, 2000);
    } else {
        $("#groupCateList").val(cateID);
    }
}
var cateID = "";
...
//    })
    $.ajax({
        ...
        success: function (data) {
            ...
            cateID = data.rows[0].category_id;
            $("#groupCateList").val(cateID).chosen(); ;
            ...
            $("#txt_summary").val(data.rows[0].summary.replace(reg, "\r\n"));
            $("#txt_description").val(data.rows[0].description.replace(reg, "\r\n"));
            ...
            $("#groupCateList").change(function () { 
                ...
                if (vgroupCate != "") {
                    $("#msgCate").hide();
                           ...
                           if (data.rows[0].member_nbr > 2) {
                               $("#input_6").removeAttr("disabled");
                           }
        }
    });
...
function cancelEdit() {
    existgroup(groupid, cancelEditT);
}
