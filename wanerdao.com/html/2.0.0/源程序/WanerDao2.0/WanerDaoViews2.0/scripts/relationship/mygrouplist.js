...
function databind(data) {
    ...
    if (data.result && data.rows) {
        ...
        $.each(data.rows, function (i, v) {
            ...
            var html = "<li><div class='lMr3'><a href=\"javascript:ifjoingroup('" + v.id + "','relationship_mygroup_info.html')\"><img src='../" + lpath + "' alt='' /></a></div>" +
					   ...
					   "<div class='rm_Meta'>圈子分类：<i>" + v.category_name + "</i></div></div>"
...
function quitGroup(id) {
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        data: "{opertype:'delorleavegroup',group_id:'" + id + "'}",
        error: function (data) {
         //   alert("error")
        },
        success: function (data) {
            if (data.msg == "isDelete") {
                $.ajax({
                    url: "../wandao_group.axd",
                    type: "POST",
                    dataType: "json",
                    ...
                    success: function (data) {
                        if (data.result) {
                            new pop({ typename: 'success', msginfo: wanerdaoLangTip('relationship_00040') });
                            pagination("1");
                        } else {
                            new pop({ typename: 'warning', msginfo: wanerdaoLangTip('relationship_00041') });
                        }

                    }
                });
            } else {
                if (data.rows != "" && data.rows[0].level == "0") {
                    new pop({ typename: 'warning', msginfo: wanerdaoLangTip('relationship_00015') });
                    // $("#msgDIV").html();
                } else {
                    $.ajax({
                        url: "../wandao_group.axd",
                        ...
                        success: function (data) {
                            if (data.result) {
                                new pop({ typename: 'success', msginfo: wanerdaoLangTip('relationship_00016') });
                                //$("#msgDIV").html(wanerdaoLangTip('relationship_00016'));
                                pagination("1");
                            } else {
                                new pop({ typename: 'warning', msginfo: wanerdaoLangTip('relationship_00017') });
