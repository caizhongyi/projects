...
function showAlbumVideo(alId, albName) {
    ...
    $('.vedio-info').empty();
    $('.flashplayer').empty(); $('.replay-content').remove();
...
function setVideo(model) {
    videoId = model.id; $('.replay-content').remove();
    ...
    $(".icon-talk").unbind("click").click(function () {
        $(".info-opt").replycontent({
            ...
            getreply:
            {
                ...
                getreplyop: 'videosinglemessage'//通过replylistbyid和axd处理获取回复列表，此属性不允许为空    
            },
            deletereply:
            {
                delreplylistbyaxd: 'replay_common.axd', //通过replylistbyid和axd处理获取回复列表，此属性不允许为空
                delreplyop: 'videodelsinglemessage'//通过replylistbyid和axd处理获取回复列表，此属性不允许为空    
            },
            addreply:
            {
                addreplylistbyaxd: 'replay_common.axd', //通过replylistbyid和axd处理添加回复信息，此属性不允许为空
                addreplyop: 'videoaddsinglemessage'//通过replylistbyid和axd处理获取回复列表，此属性不允许为空    
            },
