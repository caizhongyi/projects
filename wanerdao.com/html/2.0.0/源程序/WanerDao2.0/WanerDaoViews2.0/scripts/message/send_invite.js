        ...
        $.each(data.rows, function (i, v) {
            ...
            if (v.msg_type == "1") {//圈子
                ...
                li.append('<dl class="fInfo">'
                            + '<dt><i class="date">' + dateStr(v.send_date) + '</i><i>我邀请<a href="' + (v.send_type == 1 ? '/relationship/relationship_mygroup_info.html?id=' : '/personal/index.html?id=') + v.send_id + '">' + v.send_username + '</a>' + (v.send_type == 1 ? '的所有成员' : '') + '加入 <a href="#" class="fb">' + jsonOBJ.group_name + '</a>  (<b class="red">' + jsonOBJ.member_nbr + '人</b>)</i></dt>'
                ...
                li.append('<dl class="fInfo">'
                            + '<dt><i class="date">' + dateStr(v.send_date) + '</i><i>我请求与<a href="/personal/index.html?id=' + jsonOBJ.id + '" class="fb">' + jsonOBJ.name + '</a>成为好友' + (jsonOBJ.place ? '[' + jsonOBJ.place + ']' : '') + '</i></dt>'
