...
function databind(data) {
    ...
    if (data.rows && data.total > 0) {
        $.each(data.rows, function (i, v) {
            //        <tr>
            //			<td width="28" class="actCheck"><input type="checkbox" /></td>
            //			<td valign="top" width="62"><img class="bd_img" src="../images/default/32-32.jpg" alt="" /></td>
            //			<td>
            //			<dl class="actInfo">
            //				<dt>

            //				<a href="javascript:;" class="fb">纽约空中飞人滑雪俱乐部</a>　(<var>256</var>)
            //                <dd class="heat">
            //                    <div class="tit">关注度：</div>
            //                    <div class="score">
            //                        <i class="iBar"><i style="width:45%;"></i><em>45%</em></i>
            //                    </div>
            ...
            //                    </div>
            //                </dd>
            //				<dd class="actCon">讲回电影本身，很多人说mark扮猪吃，可是有很多细节也许不一样。<span class="actView"><a href="javascript:;">查看详细</a></span></dd>
            //			</dl>
            //			</td>
            //		</tr>
            ...
            tr_itm.append('<td><dl class="actInfo">'
            			    + '<dt>'
            			    + '<a href="/relationship/relationship_mygroup_info.html?id=' + v.id + '" class="fb" target="_blank">' + v.name + '</a>　(<var>' + v.member_nbr + '</var>)'
                            + '<dd class="heat">'
                            + '<div class="tit">关注度：</div>'
                            + '<div class="score">'
                                + '<i class="iBar"><i style="width:' + v.follow_score + '%;"></i><em>' + v.follow_score + '%</em></i>'
                            + '</div>'
                            + '<div class="tit">活跃度：</div>'
                            + '<div class="score">'
                                + '<i class="iBar"><i style="width:' + v.activity_score + '%;"></i><em>' + v.activity_score + '%</em></i>'
                            + '</div>'
                            + '</dd>'
            			    + '<dd class="actCon">' + v.desct + ' <span class="actView"><a href="/relationship/relationship_mygroup_info.html?id=' + v.id + '" target="_blank">查看详细</a></span></dd>'
            		    + '</dl>'
            		    + '</td>');
