    ...
    if (check_acc()) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data.result) {
                    if (data.msg == "3") {
                        $('#nextstep').notice(wanerdaoLangTip('acc_00019'), 1);
    ...
    $("#btnSendAgain").click(function () {
        ...
        if (!send_state) {
            $.ajax({
                ...
                error: function (data) {
                    $("#btnSendAgain").notice(wanerdaoLangTip('common_00001'), 1);
                },
                success: function (data) {
                    if (!data.result) {
                        $("#btnSendAgain").notice(data.msg, 1);
                    } else {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (!data.result) {
                    ...
                    $('#nextstep').notice(wanerdaoLangTip('acc_00023'), 2);
...
function modifyPwd() {
    if (!reset_state) {
        ...
        $.ajax({
            ...
            error: function (data) {
                $('#btnResetPwd').notice(wanerdaoLangTip('common_00001'), 1);
                reset_state = false;
            },
            success: function (data) {
                if (!data.result) {
                    $('#btnResetPwd').notice(data.msg, 1);
                    reset_state = false;
                } else {
