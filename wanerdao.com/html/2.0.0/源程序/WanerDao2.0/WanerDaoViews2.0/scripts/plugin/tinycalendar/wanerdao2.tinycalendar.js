...
* 作者：徐蓓
* 时间: 2012/7/24 22:02
* 描述：小型日历jquery插件（tooltip还没有添加），默认视图是month。需要引用jquery-1.7.2.min.js和wanerdao2.util.js
*/
    ...
    function BaseView(options) {
        ...
        function DayView() {
            ...
            function _render() {
                ...
                $datepanel.append("<div class='daypanel-val'><span class='weekname'>" + $.util.dateFormat(_now, "ddd") + "</span><span class='day'>" + _now.getDate() + "<span class='work'><a class='datepicker-tips' href='javascript:;' id='attendAct' style='display:none'>参加活动(<span id='aCount'>0</span>)</a></span></span></div>");
                ...
                var dateyear = $datepanel.find(".date-year").click(function (e) {
                    _$datepicker.find("#datepickerPop").datepicker("setDate", _now);
                    e.stopPropagation();
                    _$datepicker.find("#datepickerPop .ui-datepicker").css("width","18em");
                ...
                _getActivityCount(_now, 1, function (data) {
                    if (data.result && data.data[0] > 0) {
                        $datepanel.find("#aCount").text(data.data[0]);
                        $datepanel.find("#attendAct").show();
                    }
                });
                return $datepanel;
            }
        ...
        function WeekView() {
            ...
            function _updateCells($container) {
                ...
                var str;
                if (isSameMonth(_visStart, visEnd))
                    str = _visStart.format("MM/yy");
                else
                    str = _visStart.format("MM/yy") + "&nbsp;&nbsp;-&nbsp;&nbsp;" + visEnd.format("MM/yy");
                $container.parent().parent().find(".weekdate").html(str);
