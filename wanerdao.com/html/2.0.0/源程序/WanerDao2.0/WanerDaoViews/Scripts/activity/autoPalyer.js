...
self._showNumberRefresh();self._navRefresh(noturn);self.butPrev.attr("href","javascript:getphotoinfor('"+(self.index-1>0?self.index-1:0)+"');");self.butNext.attr("href","#id="+(self.index+1)+"");self.butPrevGroup.attr("href","javascript:getphotoinfor('"+(self.data[self.index-5>0?self.index-5:0]['id'])+"');");self.butNextGroup.attr("href","javascript:getphotoinfor('"+(self.data[self.index+6]['id'])+"');");if(null!=self.butExamine&&self.butExamine.length>0){var bigPicUrl=null;if(noturn){bigPicUrl=self.screen.attr("src");}
...
var item = null; for (var i = begin; i < end; i++) { item = self.data[i]; html += nocheck ? "<li>" : "<li" + (i == self.index ? " class=\"focus\"" : "") + ">"; html += " <a href=\"###\" onclick=\"getphotoinfor('" + self.data[i]['id'] + "');\"   id=" + self.data[i]['id'] + "  idx=" + i + " target=\"_self\"><img src=\"" + item.thumbPicUrl + "\"" + (self._isDefined(item.alt) && "" != item.alt ? "alt=\"" + item.alt + "\"" : "") + " /></a>"; html += "</li>"; }
