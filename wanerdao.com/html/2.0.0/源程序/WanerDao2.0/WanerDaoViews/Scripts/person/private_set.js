var msgTip = {
    education_null: '没有教育信息',
    work_null: '没有工作信息',
    per_name_null: '权限名为空',
    per_allow_and_refuse_null: '允许和拒绝的列表都为空',
    request_fail: '请求失败！'
}
...
$(function () {
    ...
    $('#chooseAllowUser').click(function () {
        ...
        wanerdaoPop({
            comopts: { titleid: 'common_00011', typename: 'group', id: 'ff', elementid: 'chooseAllowUser', callback: function (data) {
                $.each(data.friend, function (i, v) {
                    addPerItem('friend', v.id, v.group_name, $('#allowlist'));
                });
            }
            }
        });
    ...
    $('#chooseRefuseUser').click(function () {
        wanerdaoPop({
            comopts: { titleid: 'common_00011', typename: 'friends', id: 'ff', elementid: 'chooseRefuseUser', callback: function (data) {
                $.each(data.friend, function (i, v) {
                    addPerItem('friend', v.id, v.name, $('#refuselist'));
                });
            }
            }
        });
    ...
    $("#chooseAllowUser").overlay();
    $("#chooseRefuseUser").overlay();
});
    ...
    getCustomOfCur(function (data) {
        ...
        if (data.result && data.rows) {
            $.each(data.rows, function (i, v) {
                ...
                li_custom.append($wd.format('<label>{0}</label>', v.NAME));
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result && data.rows) {
                var box = $('#friendgroup');
                $.each(data.rows, function (i, v) {
                    var li = jQuery($wd.format('<option value="{0}">{1}</option>', v.user_id, v.name)).appendTo(box);
...
function addPerItem(type, id, name, jBox) {
    if (id && name) {
        var add = true;
        jBox.children('span').each(function () {
        ...
        if (add) {
            var item = jQuery($wd.format('<span per="{0}--{1}--{2}">{2}</span>', type, id, name)).appendTo(jBox);
            var span_remove = jQuery('<a href="javascript:;"><img src="../images/list/set_icon06.jpg"></a>').appendTo(item);
...
function saveCustomPer() {
    ...
    if (!p_name) {
        alert(msgTip.per_name_null);
        return false;
    }

    if (!p_allow && !p_refuse) {
        alert(msgTip.per_allow_and_refuse_null);
        return false;
    }
    ...
    if (p_id) {
        ...
        if (!cp_count_out) {
            ...
            alert(wanerdaoLangTip('personal_00013'));
        }
    }
}
