...
$(document).ready(function () {
    ...
    $(".accout_edit_form").validate({
        rules: {
            ...
            birthday: {
                required: wanerdaoLangTip('acc_00013'),
                dateISO: wanerdaoLangTip('acc_00013')
            }
            ...
            if ($("#iQapTcha").val()) {
                ...
                PersonRegister($("#email").val(), $("#pwd").val(), $("#truename").val(), $("input[name=sex]:checked").val(), $("#year").val(),
    ...
    //    $("#slYear,#slMonth,#slDay").change(function () {
    //        $("#birthday").val($("#slYear").val() + "-" + $("#slMonth").val() + "-" + $("#slDay").val());
    //        $("#birthday").valid();
    //    });
...
function bindSelect() {
    ...
    $('#year, #month').change(function () {
        $('#day').empty();
        lastDay = new Date($('#year').val(), $('#month').val(), 0).getDate();
        for (i = 1; i <= lastDay; i++) {
            $('#day').append($wd.format('<option value="{0}">{0}</option>', i));
        }
    });
