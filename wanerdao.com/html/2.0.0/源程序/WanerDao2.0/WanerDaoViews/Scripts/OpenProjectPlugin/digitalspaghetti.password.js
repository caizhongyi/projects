...
digitalspaghetti.password = {
    ...
    'attachWidget': function (element) {
        ...
        output.push('<span class="password-strength-bar-conatiner"><span class="password-strength-bar"></span></span>');
        if (digitalspaghetti.password.options.displayMinChar && !digitalspaghetti.password.tooShort) {
            output.push('<span class="password-min-char"></span>');
            //' + digitalspaghetti.password.options.minCharText.replace('%d', digitalspaghetti.password.options.minChar) + '
        }
        output.push('</div>');
        output = output.join('');
...
//            }
            ...
            jQuery('.password-strength-bar').animate({ opacity: 0.5 }, 'fast', 'linear', function () {
                $(".password-min-char").text(digitalspaghetti.password.strText);
                jQuery(this).css({ 'display': 'block', 'background-color': digitalspaghetti.password.strColor, 'width': digitalspaghetti.password.width + "%" }).html('&nbsp;');
