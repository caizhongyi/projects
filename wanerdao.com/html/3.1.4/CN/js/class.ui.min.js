...
(function (b) {
    ...
    Class.dialog = Class.get({initialize: function (c, d) {
        ...
        }).off("click.hidedialog").on("click.hidedialog", "[data-hide]", function (j) {
            j.stopPropagation();
            i.hide();
        });
        this.center();
        ...
        $.fn.initslimscroll && $(this.$).find('.bd').initslimscroll();
        return this
    }, hide: function () {
...
(function (a) {
    Class.floatbox = Class.get({EVENT_BEFORESHOW: "beforeshow", EVENT_AFTERSHOW: "aftershow", EVENT_ONSHOW: "onshow", EVENT_BEFOREHIDE: "beforehide", EVENT_AFTERHIDE: "afterhide", initialize: function (c, d) {
        ...
        a(window).off("resize.flaotbox").on("resize.flaotbox", function () {
            Class.floatbox.$items.each(function(){
                if( $(this).is(':visible') ){
                    var c = f.getPos($(this).data('target'));
                    $(this).css(c);
                    console.log(c)
                }
            })
        ...
        g = {left: ( d[0] == 'b' || d[0] == 't' ? f.left : h.left) , top: ( d[0] == 'l' || d[0] == 'r' ? f.top : h.top)};
        return g
