...
// WebUploader实例
    ...
    this.options = $.extend( {
        ...
        pick: {
            ...
            label: this.$.attr('label') ||   this.$.find('.file-picker').text(),
            multiple : false
        },
        formData: {
            uid: 123,
        ...
        dnd:  this.$.find('.placeholder') ,// '#dndArea',
        paste: this.$.attr('id') ,//'#uploader',
        swf: 'Uploader.swf',
        chunked: false,
        chunkSize: 512 * 1024,
        server: this.$.attr('server') || 'http://upload.273.com.cn/' ,//'../../server/fileupload.php',
        // runtimeOrder: 'flash',
        ...
        fileNumLimit:  this.$.attr('fileNumLimit') || 300 ,
        fileSizeLimit:  this.$.attr('fileSizeLimit') || 4 * 1024 * 1024,    // 200 M
        fileSingleSizeLimit: this.$.attr('fileSingleSizeLimit')   || 4 * 1024 * 1024    // 50 M
    } ,{
...
// 实例化
    uploader = WebUploader.create( this.options );
...
// 当有文件添加进来时执行，负责view的创建
    function addFile( file ) {
        var $li = $( '<li id="' + file.id + '" class="upload-piclist-item">' +
                '<p class="imgWrap"></p>'+
        ...
        $btns.on( 'click', 'span', function() {
            ...
            switch ( index ) {
                case 0:
                    ...
                    var model = JSON.parse( _this.$.find(':hidden').val() || '[]');
                    model.splice( index , 1 );
                    _this.$.find(':hidden').val( JSON.stringify(model) );

                    $li.off().find('.file-panel').off().end().remove();

                    if(  $queue.find('li').length < _this.options.fileNumLimit ){
        ...
        $coverImage = $('<span class="coverImage"></span>').appendTo( $li );
    ...
    function setState( val ) {
        ...
        switch ( state ) {
            ...
            case 'ready':
                ...
                _this.$.find( '.uploader-start' ).removeClass( 'element-invisible');
                $queue.show();
            ...
            case 'uploading':
                _this.$.find( '.uploader-start' ).addClass( 'element-invisible' );
                $progress.show();
                $upload.text( '暂停上传' );
                break;
...
// 文件上传成功，给item添加成功class, 用样式标记上传成功。
    uploader.on( 'uploadSuccess', function( file ,args ) {
        var $input = _this.$.find(':hidden'), value = JSON.parse( $input.val() || "[]");
        value.push( args )
    ...
    var files = JSON.parse( _this.$.find(':hidden').val() || '[]') ;
