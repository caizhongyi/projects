angular.module('utils.directive', ['ui.bootstrap'])
    ...
    .directive("ungDatepicker", ["$filter", "$parse", function ($filter, $parse) {
        return {
            require: '?^ngModel',
            replace: true,
            template: '<input  type="datetime"  class="form-control" placeholder="日/月"  datepicker-popup="yyyy-MM-dd" datepicker-options="dateOptions"  current-text="今天" clear-text="清空" close-text="关闭"  readonly  />',
            transclude: true,
 ...
 * */
    .directive("seniorTable", ['$state', '$modal', 'PAGINATION', '$stateParams', "$filter", "$timeout", "utils", "security",
        function ($state, $modal, PAGINATION, $stateParams, $filter, $timeout, utils, security) {
            return {
                require: '?ngModel',
                link: function ($scope, elm, attrs, ctrl) {
                ...
                var Series = {
                    init: function () {
                        ...
                        var defaults = {
                            extraParams: {
                                brand_id: function () {
                                    return $parse('item.vehicle_brand_id')(scope);
                                }
                            },
                            scroll: true,
                            scrollHeight: 300,
                        ...
                        element.nextAll('.vehicle-outer').on('click', 'a', function () {
                            ...
                            _this.value({value: $(this).text(), series_id: $(this).data('value')})
                                .setTypes(types);
                            _this.hide();
                            scope.$apply();
                        })
     ...
     * attrs.server 'upload.php'
     * */
    .directive('webUploader', ['$http', 'MBS_DOMAIN', '$parse', function ($http, MBS_DOMAIN, $parse) {
        return {
            restrict: 'AE',
            require: '?ngModel',
            replace: true,
        ...
        <div class="btns">\
            <div  class="uploader-start"></div><div class="uploadBtn">开始上传</div>\
        </div>\
        </div>\
       <span ng-transclude></span>\
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                function removeFile( file ) {
                    ...
                    updateTotalProgress();
                    $parse( ngModel ).assign( scope , $parse( ngModel )( scope).splice( $li.index() , 1 ) );
                ...
                uploader.on( 'uploadSuccess', function( file ,args ) {
                    $parse( attrs.ngModel ).assign( scope , args );
                    scope.$apply();
                });
