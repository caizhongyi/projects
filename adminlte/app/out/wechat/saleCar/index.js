angular.module('out.wechat.saleCar', [
    'out.wechat.saleCar.service',
    'ui.router'
])
    .controller('controller.out.wechat.saleCar',
    ["$scope", "$rootScope", "$stateParams", "$modal", "$log", "sale", "$state", "utils", "modules", "PAGINATION",
        function ($scope, $rootScope, $stateParams, $modal, $log, sale, $state, utils, modules, PAGINATION) {
            $scope.searchData = {};
            ...
            function getInfo() {
                sale.get(currentPage, pageSize, $scope.searchData).success(function (res) {
            ...
            getInfo();
            $scope.saleCarInfo = function () {
                $state.go('out-wechat-saleDetails', {id: this.item.id});
            }
            $scope.buyCarInfo = function () {
                $state.go('out-wechat-buyDetails', {id: this.item.id});
            }
            $scope.shareCarInfo = function () {
                $state.go('out-wechat-shareDetails', {id: this.item.id});
            }
            $scope.edit = function () {
            ...
            $scope.finish = function () {
                var item = this.item;
                sale.del(item).success(function (res) {
                    $modal.open({
    ...
    .controller('controller.modal.instance', [
        "$scope", "$rootScope", "$modalInstance", "item", "sale", "$state", "currentPage",
        function ($scope, $rootScope, $modalInstance, item, sale, $state, currentPage) {
            $scope.item = item;
            //获取Token;
            ...
            $scope.create = function (isValid) {
                if (isValid) {
                    sale.add($scope.item).success(function (res) {
            ...
            $scope.remove = function () {
                if (confirm("是否要删除？"))
                //var item = this.item ;
                    sale.remove(this.item.date).success(function (res) {
                        if (res.code == 0) {
