 ...
 */
var appUser = angular.module('change.carfee.index', ['change.carfee.index.service', 'change.carfee.index.filter', 'utils.service']);
...
appUser.controller('controller.change.carfee.index',function($scope, $rootScope, carfees, $stateParams, $state, $modal, userStatus , utils,$filter,area  ){
    ...
    function getInfo() {
        carfees.get( currentPage, pageSize , $stateParams).success(function( res ) {
            if (res.code == 0) {
                $scope.list = res.data.carfeeList;
...
appUser.directive('removeItem', function($timeout, carfees) {
    return {
        restrict : 'A',
        link : function(scope, element, attr) {
            element.click(function() {
                if (confirm("确定删除？")) {
                    carfees.remove( scope.item.product_id ).success(function( res ){
                        if( res.code == 0){
...
appUser.controller('controller.modal.instance', function ($scope, $modalInstance, $state, carfees, item, userStatus) {
    $scope.item = item;
    ...
    $scope.create = function (isValid) {
        ...
        if (isValid) {
            ...
            carfees.add( $scope.item ).success(function( res ){
                if( res.code == 0 ){
    ...
    $scope.update = function(form) {
        ...
        if (isValid) {
            ...
            carfees.update(  $scope.item ).success(function( res ){
                if( res.code == 0 ){
                    $modalInstance.dismiss('cancel');
                    $state.reload();//刷新当前页面
