...
angular.module('change.appraise.index', ['change.appraise.index.service', 'change.appraise.index.filter', 'utils.service'])
    .controller('controller.change.appraise.index',function( $scope, $rootScope, $stateParams, $modal ,$log ,appraiseinfos , $state, area,$filter , utils){
    ...
    function getInfo(){
        appraiseinfos.get( currentPage ,  pageSize  , $stateParams ).success(function( res ){
           //console.log( res.data.feebackList);

            if( res.code == 0 ){
                $scope.list = res.data.evaluateList;
...
.controller('controller.modal.instance', function ($scope, $modalInstance ,item , appraiseinfos , $state) {
    $scope.item = item;
