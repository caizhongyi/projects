 ...
 *
 * */
angular.module('acl.index.getrolelist.service',[])
    .service('roles', [ "$http" , "MBS_DOMAIN" ,"OUT_DOMAIN" ,function( $http , MBS_DOMAIN, OUT_DOMAIN){
        this.get = function( page , key , pageSize ) {
            return $http.get(MBS_DOMAIN + '/ACL/Index/getRoleList/'+ ((page) || 0 ) +'/'+ (key || 0 ) +'/'+ (pageSize || 10 ))
        }
        ...
        this.getCodes = function( id ){
            return $http.get(OUT_DOMAIN + '/vadmin/Manage/index/getOutModuleName/'+id);
        }
        this.getOutCodes = function(name){
            return $http.get(OUT_DOMAIN + '/vadmin/Manage/index/getCodesBy/?modules='+ name );
        }
}])
