 ...
 *
 * */
angular.module( 'basicservice.App.bindList.service',[])
    .service('transferA', [ "$http", "MBS_DOMAIN" , function( $http , MBS_DOMAIN){
        this.get = function( currentPage , pageSize ,  searchData ){
            ...
            return $http.post( MBS_DOMAIN + '/Basicservice/bind/bingList/' + currentPage + '/'+ pageSize ,searchData);
        }
        this.update = function( $scope ){
        	console.info($scope);
            return $http.post( MBS_DOMAIN + '/Basicservice/bind/unBingList/' , $scope);
        }
    }])
    .service("area", ["$http", "MBS_DOMAIN", function ($http, MBS_DOMAIN) {
        var MBS_DOMAIN = "http://open.273.cn";
        ///////var MBS_DOMAIN = "http://apipass.273.com.cn";
