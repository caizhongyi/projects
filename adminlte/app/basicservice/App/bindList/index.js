angular.module('basicservice.App.bindList.config', [ 'basicservice.App.bindList.service'])
    .config(["$provide",function($provide) {
        $provide.value('bindtrunkB', function() {
            return [
                {type:0, name: "云之讯"},
                {type:5, name: "轻码云"},
            ];
        ...
        $provide.value('sourceList', function() {
            return [
...
angular.module('basicservice.App.bindList', [ 'basicservice.App.bindList.service',"basicservice.App.bindList.config","basicservice.App.bindList.filter"])
    .controller('controller.basicservice.App.bindList',
    [ "$scope", "$rootScope", "$stateParams", "$modal" ,"$log" ,"transferA" , "$state" , "utils"  ,"PAGINATION","$cookieStore", "bindtrunkB", "sourceList", "issync", "$sce",
        function( $scope,$rootScope, $stateParams, $modal ,$log ,transferA , $state , utils  ,PAGINATION, $cookieStore, bindtrunkB, sourceList, issync, $sce ){
            $scope.searchData      = {} ;
            ...
            function getInfo(){
            	...
            	transferA.get( currentPage ,  pageSize ,  $scope.searchData ).success(function( res ){
            ...
            $scope.confirm = function(){
                ...
                $modal.open({
                    ...
                    controller:   [  "$scope", "$rootScope", "$state", "$filter", "$modalInstance",
                    	...
                    	$scope.closeMobile = identify.mobile;
                    	$scope.closeSource = identify.source;
                        ...
                        $scope.ok = function () {
                        	...
                        	var mobile = $('#confirm_date').val();
                        	var source = $('#confirm_source').val();
                        	var item   = [];
                        	item.mobile = mobile;
                        	item.source = source;
                        	
                            if( mobile && source ) {
                            	transferA.update( item ).success(function( res ){
                                if( res.data == '0' ){
