angular.module('basicservice.App.dosageRemain.config', [ 'basicservice.App.dosageRemain.service'])
    .config(["$provide",function($provide) {
        $provide.value('channels', function() {
            return [
                {type:'ucpaas', channel: "云之讯"},
                {type:'qingmayun', channel: "轻码云"},
                {type:'future', channel: "未来无线"},
                {type:'industry', channel: "国度行业"},
                {type:'investment', channel: "国度招商"},
                {type:'train', channel: "国度培训"},
            ];
        });
        
        $provide.value('channelM', function() {
            return [
                ...
                {type:'future', channel: "未来无线", relay: "", first:"", number:""},
                {type:'industry', channel: "国度行业", relay: "", first:"", number:""},
                {type:'investment', channel: "国度招商", relay: "", first:"", number:""},
                {type:'train', channel: "国度培训", relay: "", first:"", number:""},
            ];
        });
        
        $provide.value('typeTranslate', function() {
            return [
                
                {type:'1', channel: "短信"},
                {type:'2', channel: "回拨"},
                {type:'3', channel: "400号码"},
                {type:'4', channel: "车鉴定"},
            ];
        });
    }]);
angular.module('basicservice.App.dosageRemain', [ 'basicservice.App.dosageRemain.service',"basicservice.App.dosageRemain.config","basicservice.App.dosageRemain.filter"])
    .controller('controller.basicservice.App.dosageRemain',
    [ "$scope", "$rootScope", "$stateParams", "$modal" ,"$log" ,"counts" , "$state" , "utils"  ,"PAGINATION","$cookieStore" ,
        function( $scope,$rootScope, $stateParams, $modal ,$log ,counts , $state , utils  ,PAGINATION, $cookieStore ){
            
            function getInfo(){
            	counts.get().success(function( res ){
                    $scope.list = res.data;
                    $(window).resize();
                })
            }
