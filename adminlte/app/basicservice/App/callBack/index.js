angular.module('basicservice.App.callBack.config', [ 'basicservice.App.callBack.service'])
    .config(["$provide",function($provide) {
    	$provide.value('bindtrunk', function() {
            return [
                {type:"ucpaas", name: "云之讯"},
                {type:"qingmayun", name: "轻码云"},
                {type:"ytx", name: "云通讯"},
                {type:"yican", name: "亿灿"},
            ];
        });
    	
    	$provide.value('channelTransform', function() {
            return [
                {type:"ucpaas", name: "ucpaascall"},
                {type:"qingmayun", name: "qingmayun"},
            ];
        });

    	$provide.value('isdown', function() {
            return [
                {type:'0', name: "未下载"},
                {type:'1', name: "下载中"},
                {type:'2', name: "已下载"},
...
angular.module('basicservice.App.callBack', [ 'basicservice.App.callBack.service',"basicservice.App.callBack.config","basicservice.App.callBack.filter"])
    .controller('controller.basicservice.App.callBack',
    [ "$scope", "$rootScope", "$stateParams", "$modal" ,"$log" ,"transfer" , "$state" , "utils"  ,"PAGINATION","$cookieStore" , "bindtrunk", "issync", "channelTransform", "$sce",
        function( $scope,$rootScope, $stateParams, $modal ,$log ,transfer , $state , utils  ,PAGINATION, $cookieStore, bindtrunk, issync, channelTransform, $sce ){
            $scope.searchData      = {} ;
            ...
            $stateParams.currentPage = $stateParams.currentPage || PAGINATION.currentPage;
            $stateParams.pageSize    = pageSize;
            $scope.searchData = $stateParams ;
            
            $scope.sourceList = bindtrunk();
            $scope.hangupList = issync();
            $scope.mp3Url     = 'http://218.5.2.188:45912';
            $scope.mbsUrl     = 'http://mbs.273.cn';
            
            //$scope.downmp3    = 'http://apipass.273.com.cn';
            $scope.downmp3    = 'http://open.273.cn';
            $scope.baseUrl    = 'http://www.273.cn/car';
            $scope.iframM3    = 'http://mbs.273.cn/app/basicservice/Transfer/transferList/index-m3.php';
            
            var isChange = false;
            function getInfo(){
                transfer.get( currentPage ,  pageSize ,  $scope.searchData ).success(function( res ){
            ...
            $scope.play = function(){
                this.identify.src = 'http://218.5.2.188:45912/'+ this.identify.file_path +'.mp3';
            }
            ...
            $scope.whatClassShow= function(dept_name, real_name, flag) {
                if(dept_name != '' || real_name != '') {
                	if(flag != '1') {
                		return true;
                	}
                	return false;
            ...
            $scope.confirm = function(){
            	...
            	var cTransform = channelTransform();
            	
            	for (var indexer in cTransform) {
                    if (cTransform[indexer].type == identify.channel) {
                    	var Root = cTransform[indexer].name;
                    }
                }
            	
            	var m3       = $scope.iframM3+'?url='+$scope.mp3Url+'/'+identify.format_record_url;
                ...
                $modal.open({
                    ...
                    controller:   [  "$scope", "$rootScope", "$state", "$filter", "$modalInstance",
                        ...
                        $scope.m3url          = $sce.trustAsResourceUrl(m3);
                        $scope.idA            = identify.id;
                        $scope.callerA        = identify.caller;
                        $scope.calledA        = identify.called;
                        //$scope.forward_extnoA = identify.forward_extno;
            ...
            else if(obj.click != null){obj.click();} else{SelectDater(obj,strFormat,x,y)} } 

            //选择日期 → 由 寒羽枫 2006-06-25 添加 
            $scope.SelectDater = function(ids, strFormat,x,y) 
            {
            ...
            */ 
            ...
            { 
            	...
            	var dates_id = calendar.dateControl.id;
                if(dates_id == 'start_dateA') {
                	$scope.searchData.start_date = '';//为 $scope.start_date 赋值
                } else {
                	$scope.searchData.end_date = '';//为 $scope.end_date 赋值
                }
            } 
            ...
            var dates_id = calendar.dateControl.id;
            if(dates_id == 'start_dateA') {
            	$scope.searchData.start_date = today.format(calendar.dateFormatStyle);//为 $scope.start_date 赋值
            } else {
            	$scope.searchData.end_date = today.format(calendar.dateFormatStyle);//为 $scope.end_date 赋值
            }
