angular.module('basicservice.ExtPhone.cdrList.config', [ 'basicservice.ExtPhone.cdrList.service'])
    .config(["$provide",function($provide) {
        $provide.value('trunk', function() {
            return [
                {type:63089200, name: "WEB站"},
                {type:63089201, name: "M站"},
                {type:63089202, name: "APP站"},
                {type:63089203, name: "全网老"},
                {type:63089204, name: "百姓"},
                {type:63089206, name: "58"},
                {type:63089207, name: "赶集"},
                {type:63089208, name: "爱卡"},
                {type:63089277, name: "优车到"},
                {type:63089273, name: "客服中心"},
                {type:63089288, name: "测试"},
            ];
        });

        $provide.value('hangup', function() {
            return [
...
angular.module('basicservice.ExtPhone.cdrList', [ 'basicservice.ExtPhone.cdrList.service',"basicservice.ExtPhone.cdrList.config","basicservice.ExtPhone.cdrList.filter"])
    .controller('controller.basicservice.ExtPhone.cdrList',
    [ "$scope", "$rootScope", "$stateParams", "$modal" ,"$log" ,"cdr" , "$state" , "utils"  ,"PAGINATION","$cookieStore" , "hangup", "trunk", "issync", "$sce",
        function( $scope,$rootScope, $stateParams, $modal ,$log ,cdr , $state , utils  ,PAGINATION, $cookieStore, hangup, trunk, issync, $sce ){
            $scope.searchData      = {} ;
            $scope.maxSize         = PAGINATION.maxSize;
            $scope.searchCondition = $scope.searchCondition || {};
            //PAGINATION.maxSize = PAGINATION.pageSize = 25;
            var pageSize           =  25 ;
            var currentPage        =  $stateParams.currentPage || PAGINATION.currentPage;

            $stateParams.pageSize    = pageSize;
            $scope.searchData = $stateParams ;
            $scope.hangupList = hangup();
            $scope.trunkList  = trunk();
            $scope.syncList   = issync();
            $scope.domainUrl  = 'http://open.273.cn';
            $scope.mp3Url     = 'http://218.5.2.188:45912';
            $scope.mbsUrl     = 'http://mbs.273.cn';
            $scope.baseUrl    = 'http://www.273.cn/car';
            $scope.iframM3    = 'http://mbs.273.cn/app/basicservice/ExtPhone/cdrList/index-m3.php';
            //$scope.searchData.pageSize = pageSize;
            var isChange = false;
            function getInfo(){
                if( isChange ) $scope.currentPage = 1 ;
                cdr.get( currentPage ,  pageSize ,  $scope.searchData ).success(function( res ){
                	$scope.list = res.data.list;
                    $scope.totalItems = res.data.total;
                    //$scope.numPages = Math.ceil(res.data.total / 25);
            ...
            $scope.confirm = function(){
            	...
            	var m3       = $scope.iframM3+'?url='+$scope.mp3Url+'/'+identify.file_path+'.mp3';
                $modal.open({
