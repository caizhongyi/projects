 ...
 * ?
 */
angular.module('settlement.apply.applyFee.servicefeeapply', ["settlement.apply.applyFee.service"])
        ...
        function ($scope, $rootScope, $stateParams, $modal, $state, applyFee,  $filter, $cookieStore , $timeout , utils ) {
            applyFee.get( $stateParams ).success(function( res ){
                if (res.code == 0) {
                    ...
                    applyFee.avoidCode( '2' , $scope.item.order_no ).success(function( res ){
                        if(res.code == 0){
                    ...
                    $scope.subitem.is_jhc = $scope.item.is_jhc;
                }
                else {
            ...
            $scope.$watch(function(){
                $scope.subitem.fee = parseFloat( $scope.subitem.other_fee || 0 ) + parseFloat( $scope.subitem.transfer_fee || 0) + parseFloat( $scope.subitem.anency_fee || 0) + parseFloat( $scope.subitem.brokerage || 0)
            });
            ...
            $scope.submit = function( isValid ){
                ...
                if( isValid ){
                    ...
                    applyFee.serviceFeeApply( $scope.subitem ).success(function ( res ) {
                        $timeout(function(){
                            $scope.submitted = false;
                            if (res.code == 0) {
