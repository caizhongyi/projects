 ...
 * ?
 */
angular.module('trade.index.edittradedetail', ["trade.index.edittradedetail.service"])
    .controller('controller.trade.index.edittradedetail',
    [ "$scope", "$rootScope", "$stateParams", "$modal" , "$state" , "area" , "utils" , "tradeDetail" , "$filter", "$cookieStore",
        function( $scope,$rootScope, $stateParams, $modal , $state , area , utils , tradeDetail , $filter, $cookieStore ){
        ...
        $scope.trade = {} ;
        if( $stateParams.id ){
            tradeDetail.getById( $stateParams.id  ).success(function( res ){
                var d = res.data.sale_date;
                $scope.trade = res.data ;
                console.log("zbytest1");
            ...
            tradeDetail.get( null , userInfo.id  ).success(function( res ){
                console.log("zbytest2");
                if(res.code == 1){
                    alert(res.msg);
                    console.log("zbytest3");
                    $state.go('trade-index-index')

                }
                else{
                    console.log("zbytest4");
                    $scope.trade = res.data ;
                    console.log($scope);
                    $scope.trade.seller_cert_type = $scope.trade.buyer_cert_type = "身份证"
                }
        ...
        $scope.submit = function(isValid) {
            ...
            if (!isValid) {
                ...
                serializeTrade();
                tradeDetail.update( $scope.trade ).success(function( res ) {
                    if( res.code == 0 ){
        ...
        $scope.save = function(){
           serializeTrade();
           tradeDetail.draft( $scope.trade ).success(function( res ) {
                if( res.code == 0 ){
                    utils.message("保存成功！");
                }
                else{
