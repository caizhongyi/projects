var _TD = {a: [], init: function (c, d) {
    delete this.init;
    var b, a = {version: "0.1.16", is_debug: !!d, is_paused: true, width: 16, height: 16, show_monster_life: true, fps: 0, exp_fps: 24, exp_fps_half: 12, exp_fps_quarter: 6, exp_fps_eighth: 4, stage_data: {}, defaultSettings: function () {
        ...
        var g = a.lang.$e("wrapper"), f = h.clientX - g.offsetLeft - this.canvas.offsetLeft + Math.max(document.documentElement.scrollLeft, document.body.scrollLeft), i = h.clientY - g.offsetTop - this.canvas.offsetTop + Math.max(document.documentElement.scrollTop, document.body.scrollTop);
        return[f, i]
    }, hover: function (e, f) {
...
_TD.a.push(function (a) {
    a.lang = {$e: function (b) {
        ...
        var e = Math.floor(Math.random() * 256), d = Math.floor(Math.random() * 256), c = Math.floor(Math.random() * 256);
...
_TD.a.push(function (a) {
    a.eventManager = {ex: -1, ey: -1, _registers: {}, ontypes: ["enter", "hover", "out", "click"], current_type: "hover", isOn: function (b) {
...
_TD.a.push(function (a) {
    ...
    a.Stage.prototype = {_init: function () {
...
_TD.a.push(function (a) {
    ...
    a.Act.prototype = {_init: function () {
...
_TD.a.push(function (a) {
    ...
    a.Scene.prototype = {_init: function () {
...
_TD.a.push(function (a) {
    ...
    a.Element.prototype = {_init: function () {
        var e = this, d, c, b;
    ...
    }, onEnter: a.lang.nullFunc, onOut: a.lang.nullFunc, onHover: a.lang.nullFunc, onClick: a.lang.nullFunc, step: a.lang.nullFunc, render: a.lang.nullFunc, addToScene: function (d, b, c, e) {
        this.scene = d;
        if (isNaN(b)) {
            return
        }
...
_TD.a.push(function (a) {
    ...
    var d = {_init: function (h) {
        ...
        this.pre_building = new a.Building(this.id + "-pre-building-" + a.lang.rndStr(), {type: g, map: this, is_pre_building: true});
        ...
        if (typeof g == "number") {
            g = new a.Monster(null, {idx: g, difficulty: a.difficulty, step_level: this.step_level, render_level: this.render_level + 2})
        }
...
_TD.a.push(function (a) {
    var b = {_init: function (c) {
        ...
        var d, e = this, c = new a.FindWay(this.map.grid_x, this.map.grid_y, this.map.entrance.mx, this.map.entrance.my, this.map.exit.mx, this.map.exit.my, function (f, g) {
        ...
        var d = new a.Building("building-" + c + "-" + a.lang.rndStr(), {type: c, step_level: this.step_level, render_level: this.render_level});
        d.locate(this);
...
_TD.a.push(function (a) {
    var c = {_init: function (d) {
        ...
        new a.Bullet(null, {building: this, damage: this.damage, target: this.target, speed: this.bullet_speed, x: d, y: f})
    }, tryToFire: function () {
        ...
        var e = this._upgrade_records[f], d = "max_" + f, g = "_upgrade_rule_" + f, h = this[g] || a.default_upgrade_rule;
        if (!e || isNaN(e)) {
            return
        }
        e = h(this.level, e);
    ...
    var b = {_init: function (d) {
        ...
        if (e) {
            ...
            this.is_valid = false;
            a.Explode(this.id + "-explode", {cx: this.cx, cy: this.cy, r: this.r, step_level: this.step_level, render_level: this.render_level, color: this.color, scene: this.map.scene, time: 0.2});
            return true
        }
        return false
    }, step: function () {
...
_TD.a.push(function (a) {
    var b = {_init: function (e) {
        ...
        var g = this, d = new a.FindWay(this.map.grid_x, this.map.grid_y, this.grid.mx, this.grid.my, this.map.exit.mx, this.map.exit.my, function (h, i) {
        ...
        if (this.cx == this.next_grid.cx && this.cy == this.next_grid.cy) {
            ...
            var d = this.next_grid.cx - this.cx, h = this.next_grid.cy - this.cy, g = d < 0 ? -1 : 1, f = h < 0 ? -1 : 1, e = this.speed * a.global_speed;
...
_TD.a.push(function (a) {
    var d = {_init: function (f) {
        f = f || {};
        this.x = f.x;
        this.y = f.y;
        this.scene = f.scene;
        this.map = f.main_map;
        var g = new a.Map("panel-map", a.lang.mix({x: this.x + f.map.x, y: this.y + f.map.y, scene: this.scene, step_level: this.step_level, render_level: this.render_level}, f.map, false));
        this.addToScene(this.scene, 1, 7);
        g.addToScene(this.scene, 1, 7, g.grids);
        this.scene.panel_map = g;
        this.gameover_obj = new a.GameOver("panel-gameover", {panel: this, scene: this.scene, step_level: this.step_level, is_visiable: false, x: 0, y: 0, width: this.scene.stage.width, height: this.scene.stage.height, render_level: 9});
        this.balloontip = new a.BalloonTip("panel-balloon-tip", {scene: this.scene, step_level: this.step_level, render_level: 9});
        this.balloontip.addToScene(this.scene, 1, 9);
        this.btn_pause = new a.Button("panel-btn-pause", {scene: this.scene, x: this.x, y: this.y + 260, text: a._t("button_pause_text"), step_level: this.step_level, render_level: this.render_level + 1, onClick: function () {
            if (this.scene.state == 1) {
                this.scene.pause();
        ...
        this.btn_restart = new a.Button("panel-btn-restart", {scene: this.scene, x: this.x, y: this.y + 300, is_visiable: false, text: a._t("button_restart_text"), step_level: this.step_level, render_level: this.render_level + 1, onClick: function () {
            setTimeout(function () {
                a.stage.clear();
                a.is_paused = true;
                a.start();
        ...
        this.btn_upgrade = new a.Button("panel-btn-upgrade", {scene: this.scene, x: this.x, y: this.y + 300, is_visiable: false, text: a._t("button_upgrade_text"), step_level: this.step_level, render_level: this.render_level + 1, onClick: function () {
            this.scene.map.selected_building.tryToUpgrade(this)
        }});
        this.btn_sell = new a.Button("panel-btn-sell", {scene: this.scene, x: this.x, y: this.y + 340, is_visiable: false, text: a._t("button_sell_text"), step_level: this.step_level, render_level: this.render_level + 1, onClick: function () {
...
_TD.a.push(function (a) {
    ...
    a.getDefaultStageData = function (d) {
        var e = {stage_main: {width: 640, height: 560, init: c, step2: b}, scene_endless: {map: {grid_x: 16, grid_y: 16, x: a.padding, y: a.padding, entrance: [0, 0], exit: [15, 15], grids_cfg: [
            {pos: [3, 3], passable_flag: 0},
            {pos: [7, 15], build_flag: 0},
            {pos: [4, 12], building: "wall"},
            {pos: [4, 13], building: "wall"}
        ]}, panel: {x: a.padding * 2 + a.grid_size * 16, y: a.padding, map: {grid_x: 3, grid_y: 3, x: 0, y: 110, grids_cfg: [
            {pos: [0, 0], building: "cannon"},
            {pos: [1, 0], building: "LMG"},
            {pos: [2, 0], building: "HMG"},
            {pos: [0, 1], building: "laser_gun"},
            {pos: [2, 2], building: "wall"}
        ]}}, config: {endless: true, wait_new_wave: a.exp_fps * 3, difficulty: 1, wave: 0, max_wave: -1, wave_damage: 0, max_monsters_per_wave: 100, money: 500, score: 0, life: 100, waves: [
            [],
            [
                [1, 0]
            ],
...
_TD.a.push(function (a) {
    ...
    a.getDefaultBuildingAttributes = function (b) {
        var c = {wall: {damage: 0, range: 0, speed: 0, bullet_speed: 0, life: 100, shield: 500, cost: 5}, cannon: {damage: 12, range: 4, max_range: 8, speed: 2, bullet_speed: 6, life: 100, shield: 100, cost: 300, _upgrade_rule_damage: function (e, d) {
            return d * (e <= 10 ? 1.2 : 1.3)
        }}, LMG: {damage: 5, range: 5, max_range: 10, speed: 3, bullet_speed: 6, life: 100, shield: 50, cost: 100}, HMG: {damage: 30, range: 3, max_range: 5, speed: 3, bullet_speed: 5, life: 100, shield: 200, cost: 800, _upgrade_rule_damage: function (e, d) {
            return d * 1.3
...
_TD.a.push(function (a) {
    ...
    a.getDefaultMonsterAttributes = function (f) {
        var d = [
            {name: "monster 1", desc: "\u6700\u5f31\u5c0f\u7684\u602a\u7269", speed: 3, max_speed: 10, life: 50, damage: 1, shield: 0, money: 5},
            {name: "monster 2", desc: "\u7a0d\u5f3a\u4e00\u4e9b\u7684\u5c0f\u602a", speed: 6, max_speed: 20, life: 50, damage: 2, shield: 1},
            {name: "monster speed", desc: "\u901f\u5ea6\u8f83\u5feb\u7684\u5c0f\u602a", speed: 12, max_speed: 30, life: 50, damage: 3, shield: 1},
            {name: "monster life", desc: "\u751f\u547d\u503c\u5f88\u5f3a\u7684\u5c0f\u602a", speed: 5, max_speed: 10, life: 500, damage: 3, shield: 1},
            {name: "monster shield", desc: "\u9632\u5fa1\u5f88\u5f3a\u7684\u5c0f\u602a", speed: 5, max_speed: 10, life: 50, damage: 3, shield: 20},
            {name: "monster damage", desc: "\u4f24\u5bb3\u503c\u5f88\u5927\u7684\u5c0f\u602a", speed: 7, max_speed: 14, life: 50, damage: 10, shield: 2},
            {name: "monster speed-life", desc: "\u901f\u5ea6\u3001\u751f\u547d\u90fd\u8f83\u9ad8\u7684\u602a\u7269", speed: 15, max_speed: 30, life: 100, damage: 3, shield: 3},
            {name: "monster speed-2", desc: "\u901f\u5ea6\u5f88\u5feb\u7684\u602a\u7269", speed: 30, max_speed: 40, life: 30, damage: 4, shield: 1},
            {name: "monster shield-life", desc: "\u9632\u5fa1\u5f88\u5f3a\u3001\u751f\u547d\u503c\u5f88\u9ad8\u7684\u602a\u7269", speed: 3, max_speed: 10, life: 300, damage: 5, shield: 15}
        ];
...
_TD.a.push(function (b) {
    ...
    var a = {cannon: function (d, e, h, f, i) {
...
_TD.a.push(function (a) {
    ...
    a.FindWay.prototype = {_init: function () {
