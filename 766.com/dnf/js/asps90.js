...
function initForm(is_index_page, versions){
	...
	if(!sc) updateHashCode();//版本 hashcode 更新
	window.onhashchange = onHashChanged;//ハッシュ変更の監視を開始
}
...
SkillInfo.prototype = {
	//id, 日本語名
	initialize : function(id, name,icon){
		this.id      = id;
        this.icon   = icon;
		//this.name    = name;
		if(!TT[id]){
...
this.max_slv_tmp = max_slv;//コード変換のための作業用
		...
		if(this.max_slv>30)this.max_slv = 30;//LV20を最大値とする．
	...
	setAsAp : function(){
		...
		return this.setAsBasic(48, 30, 3).setFirstCost(0);
	},
	//覚醒アクティブテンプレ
	setAsAa : function(){
		return this.setAsBasic(50, 80, 2);
	},
	//60Exアクティブテンプレ
	setAsEx60 : function(){
		return this.setAsBasic(60, 80, 5, 3);
	},
	//70Exアクティブテンプレ
	setAsEx70 : function(max_slv){
		if(max_slv==undefined){max_slv=1;}
		return this.setAsBasic(70, 120, 5, max_slv);
	},
	...
	setAsEx : function(learn_from, tp_cost, lv_cost, max_slv, can_learn_and_raise){
		...
		if(max_slv==-1 || max_slv==undefined){
			if(tp_cost==4){
				...
				max_slv = 1; //TP3ならデフォルトMAX1
			}else{
				max_slv = 3; //TP1ならデフォルト3
			}
		}
	...
	//前提条件を追加する
	//キャンセルスキルとEXスキルは引数をひとつだけ指定可能
	//それ以外のスキルで引数をひとつだけ指定した場合は，直前に生成したスキルを前提とする．
	addPre : function(id_or_slv, slv){
		if(slv==undefined){
...
'<tr id="record_#{id}" class="#{type}">'+
	...
	'<td id="icon_#{id}" class="skill_icon" style="padding:0px 0px 0px 0px"><img src="{icon}" class=spacing_icon width={icon_width}>'+
	//'<td id="icon_#{id}" class="skill_icon" style="padding:0px 0px 0px 0px"><img src="icons/KAm/#{id}.png" class=spacing_icon width={icon_width}>'+
...
function printSkills(target, skills){
	...
	skills.each(function(si){
		si.makeOptionsHTML();
		ih += record_template_.evaluate(si).replace(/{tier}/, tt[si.tier]).replace(replace_from, replace_to).replace(/{icon}/,si.icon);
	});
	ih += "</table>";
	$(target).innerHTML = ih;
}
...
function getQpByLV(){
	...
	return 3006;//2996;//2950;//2810;//2730;
}
function getUsedQp(){
	return $F("used_qp");
}
