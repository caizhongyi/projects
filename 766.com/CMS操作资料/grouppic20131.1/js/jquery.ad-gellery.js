(function($) {
  $.fn.adGallery_modify = function(options) {
    var defaults = {
		loader_image: 'http://i3.static.olcdn.com/js/grouppic2013129/loading.gif',
        start_at_index: 0,
        description_wrapper: false,
        // update by fjc (2011-03-31)
  ...
  AdGallery.prototype = {
    ...
    init: function(wrapper, settings) {
	  ...
	  //modify by ssl
	  if( this.settings.showList.btnLB && this.settings.showList.listCNT && this.settings.showList.listItem )
	  	this.setList();
    },
      ...
      thumbs.each(
        function(i) {
          ...
          context.images[i] = { thumb: thumb.attr('src'), image: image_src, error: false,
                                ...
                                link: image_link,image_link:image_link,title2:$(this).attr('title') };
        }
      );
	...
	initScrollBar: function(){
		var sb = this;
		/*$('.ad-scrollbar').bind('drag',function( event ){
	  	var _left = (((event.offsetX-225)>=0 && (event.offsetX-225)<= 430)?(event.offsetX-225): ( (event.offsetX-225)<0?0:430 ) );
  		$( this ).css( { left: _left });

		sb.thumbs_wrapper.scrollLeft( _left/sb.sb_bili );
  }); */
	...
	setList: function(){
		var context = this;
		$( context.settings.showList.btnHD ).bind( 'click',function(){
			$( context.settings.showList.listCNT ).hide();
			$( '.ad-gallery' ).show();
		} );

		$( this.settings.showList.btnLB ).bind( 'click',function(){
    ...
    _getDescription: function(image) {
      ...
      if(image.desc.length || (image.title2 && image.title2.length)) {
        var title = '';
    ...
    showImage: function(index, callback) {
		if( index < (this.images.length-1) ){
		  (index == 0)?$( '.ad-prev' ).hide():$( '.ad-prev' ).show();
		  $( '.ad-next' ).show();
		  //$('.ad-lookpic').hide();
	  	};
      ...
      if(this.images[index] ) {
        ...
        if(image.link) {
          ...
          if( index == context.images.length-1 ){
			  if( context.settings.lastdiv != '' )
				img_container.html( $( context.settings.lastdiv ).html());
			$( '.ad-prev' ).hide();
			$( '.ad-next' ).hide();
		  }else{
		...
		if( index == context.images.length-1 ){
			...
			img_container.css( {width:'100%',left:0} );
		}else{
			img_container.css({width: size.width +'px', height: size.height +'px'});
			this._centerImage(img_container, size.width, size.height);
		};
		var desc = this._getDescription(image, img_container);
		if( context.settings.ckpic )
			$( context.settings.ckpic ).attr( { 'href' : context.images[index].image_link } );
		if(desc) {
		  if(!this.settings.description_wrapper) {
			//img_container.append(desc);
			$( '.ad-title' ).html( '<span>(<span>' + (index + 1) + '</span>/' + (this.images.length-1) + ')</span> ' + desc.html() );
			$( '.ad-title2' ).html( image.title2 + '(' + (index+1) + '/' + (this.images.length-1) + ')' );
        ...
        if(this.current_image || this.settings.animate_first_image) {
          ...
          img_container.animate(animation.new_image, animation_speed, easing,
            function() {
				$('.ad-scrollbar').css( {left: context.sb_bili*context.thumbs_wrapper.scrollLeft() +'px'});
