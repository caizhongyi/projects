var festival = function () {
    ...
    if (getdate(year + "/4/1 00:00:00") < nd.getTime() && nd.getTime() < getdate(year + "/4/1 23:59:59")) {
        document.writeln('<script src="http://js.olcdn.com/festival/fools/fools.js?v3" type="text/javascript" ></script>');
    }
    //清明节
    if (getdate(year + "/04/04 00:00:00") < nd.getTime() && nd.getTime() < getdate(year + "/04/04 23:59:59")) {
        document.writeln('<link href="http://js.olcdn.com/festival/tombsweeping/index.css" type="text/css" rel="stylesheet" />');
    }
    //劳动节
