...
var bottomAd = function(){
	//底部广告是否存在
	if($bottomAd.html() !='' && $bottomAd1.html()!=''){
		$bottomAd.hide();
		$bottomAd1.show();
...
//前弹广告函数主体
var smallThumbWidth = $(window).width()>=1330 ? 230 : 80; //前弹缩略图尺寸
var top_ad = function(){
	var $doc = $('body'),
		timerTa = timerTa2 = '',
		$topThumb = $("#top_thumb"),
    ...
    var $topad =  $('.top_ad');
    if($('#top_thumb .top-thumb-inner').html() != ""){
        $topad.stop(true).fadeIn();
    };

    timerTa = setTimeout(function(){
        $topad.stop(true).fadeOut();
        $topThumb.show();
        hasPlay = false;
        //----------底部广告开始----------//
        //bottomAd();
    ...
    $('.close',$topad).live('click',function(){
        $topad.hide();
        $topThumb.show();
    });
...
var floatAd = function(){
	...
	if(typeof(star_ad1)=='object' && n<=0){
		var $body = $('body');
        $('#star_ad1').stop(true).fadeIn().find('.close').bind('click',function(){
			$('#star_ad1').stop(true).fadeOut();
		});	
		//右下角广告3小时出现一次
