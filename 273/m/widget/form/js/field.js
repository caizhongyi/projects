...
var Validator =  window.Validator || require('validator.js')
...
var Field = Class.create({
    ...
    addRule : function (config) {
        ...
        if (config instanceof Validator.Rule) {
            ...
            name = config.name;
            delete config.name;
            rule = Validator.getRule(name, $.extend(config, {$el : this.$}));
        }
    ...
    validate : function (flag) {
        ...
        if (this.isDisabled() || (!this.isRequired() && this.isEmpty(value))) {
            return $.Deferred().done(function () {
                self.setValid(true);
                $(self.$[0]).trigger('field-done', [!flag]);
            }).resolve();
        }
        ...
        return $.when.apply($, defers)
            .done(function () {
                ...
                if (next) {
                    ...
                    $(self.$[0]).trigger('field-done');
                }
            })
            .fail(function (msg) {

                self.setValid(false);
                $(self.$[0]).trigger('field-fail', [msg]);
            })
            .always(function () {
                // todo : something
            });
