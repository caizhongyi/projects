...
Common.anchor = function($el) {
    ...
    $el.find('[data-anchor]').on('click',function(){
        ...
        if (window.parent !== window && /iphone/i.test(navigator.userAgent)) {
            require.async(['app/car/lib/messager/messager.js'], function(Messager) {
                if (!messager) {
...
Common.isCarNum = function(carNum) {
    if (!carNum) {
        return false;
    }
    var regexp = /^(([0-9a-zA-Z]{5})|([0-9A-Za-z]{4}(学|挂)))$/;
...
//是否只包含中文或英文字母
Common.onlyChineseOrEnglish = function(str) {
    var reg = /^[\u4E00-\u9FA5A-Za-z]+$/;
