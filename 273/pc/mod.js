...
(function(global) {
    ...
    function loadScript(id, callback, onerror) {
        ...
        // resource map query
        //
        var res = resMap[id] || resMap[id + '.js'] || resMap[id + '.tpl'] || {};
        var pkg = res.pkg;
        var url;
    ...
    require.loadCss = function(cfg) {
        if (cfg.content) {
            ...
            var reg = /^(http:\/\/)/i;
            if (!reg.test(cfg.url)) {
                cfg.url = require.domain + cfg.url;
            }
            link.href = cfg.url;
            link.rel = 'stylesheet';
            link.type = 'text/css';
    ...
    require.alias = function(id) {
        return id.replace(/\.(js|tpl)$/i, '');
    };

    require.timeout = 5000;

    require.domain = 'http://asset.273.com.cn';
