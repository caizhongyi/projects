...
<div class="main">
				...
				<li style="width:23.6%; min-width:240px;">
                    <div class="thumbnail">
                        ...
                        <div class="module-button">
							...
							<?php  } else { ?>
                            <?php  if($row['enabled']) { ?>
								<a id="enabled_<?php  echo $row['mid'];?>_0" href="<?php  echo create_url('member/module/enable', array('mid' => $row['mid'], 'enabled' => 0))?>" onclick="return ajaxopen(this.href)" class="btn btn-primary btn-xs module-button-switch">禁用</a>
                            <?php  } else { ?>
								<a id="enabled_<?php  echo $row['mid'];?>_1" href="<?php  echo create_url('member/module/enable', array('mid' => $row['mid'], 'enabled' => 1))?>" onclick="return ajaxopen(this.href);" class="btn btn-danger btn-xs module-button-switch">启用</a>
                            <?php  } ?>
                            <?php  } ?>
							<!--去除快捷操作-->
							<?php  if($row['settings'] && $row['enabled']) { ?>
								<a href="<?php  echo create_url('member/module/setting', array('mid' => $row['mid']))?>" class="btn btn-xs module-button-switch">设置</a>
                                    <?php  } ?>
                    </div>
                </div>
				</li>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('.module .thumbnails').delegate('li', 'mouseenter mouseleave', function(){ //控制模块详细信息
