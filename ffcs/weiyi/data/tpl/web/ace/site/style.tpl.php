<?php defined('IN_IA') or exit('Access Denied');?><?php  $_W['breadcrumb'] = array(array('title'=>'微官网'),array('title'=>'风格管理'))?>
<?php  include template('common/header', TEMPLATE_INCLUDEPATH);?>
<div class="page-header">
    <h1>
        <i class="icon-book"></i> 风格管理
    </h1>
</div>
...
<div class="main">
	<form id="form" class="form-horizontal form" action="" method="post" enctype="multipart/form-data">
		...
		<table class="tb">
            ...
            <tr>
                <td colspan="100%"><h5 class="text-info">以下为高级设置，建议系统默认设置</h5></td>
            </tr>
			<tr class="hide">
	...
	<div class="template form-horizontal">
		...
		<li>
			<div class="template-style-pic <?php  if($_W['account']['styleid'] == $item['id']) { ?>on<?php  } ?>"> <!--设为默认风格时class中加on-->
				...
				<img src="./themes/mobile/<?php  echo $item['name'];?>/preview.jpg" style="width: 154px;margin-top:55px;" />
				<span class="icon-ok"></span>
			</div>
			<div class="template-style-button">
				<a href="javascript:;" onclick="ajaxpreview('<?php  echo $item['id'];?>');return false;" class="btn btn-xs pull-right" >预览</a>
				<a href="<?php  echo create_url('site/style/default', array('templateid' => $item['id']))?>" class="btn btn-xs">设为默认</a>
				<a href="<?php  echo create_url('site/style/designer', array('templateid' => $item['id']))?>" class="btn btn-xs">设计风格</a>
			</div>
		</li>
		<?php  } } ?>
		</ul>
...
<!--
    function ajaxpreview(styleid) {
        ...
        html = '<iframe width="100%" scrolling="yes" height="100%" frameborder="0" src="<?php  echo create_url('site/preview')?>&styleid='+styleid+'" id="preview" name="preview" style="width: 320px; overflow: visible; height: 480px;"></iframe><div class="modal-footer" style="margin-top: -10px;"><a href="<?php  echo $_W['siteroot'];?><?php  echo create_url('mobile/channel', array('name' => 'index', 'weid' => $_W['weid']))?>&styleid='+styleid+'" target="preview" class="btn">首页</a><a href="<?php  echo $_W['siteroot'];?><?php  echo create_url('mobile/channel', array('name' => 'home', 'weid' => $_W['weid']))?>" target="preview" class="btn">个人中心</a><a href="#" class="btn" data-dismiss="modal" aria-hidden="true">关闭</a></div>';
