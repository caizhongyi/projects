...
<head>
    ...
    <link rel="stylesheet" href="./resource/ace/assets/css/systemhelp.css" />
    ...
    <?php  if(is_array($load_css)) { foreach($load_css as $css) { ?>
    <link type="text/css" rel="stylesheet" href="./resource/style/<?php  echo $css;?>.css"  />
    <?php  } } ?>
    <?php  if(is_array($load_js)) { foreach($load_js as $js) { ?>
    <script type="text/javascript" src="./resource/script/<?php  echo $js;?>.js" ></script>
    <?php  } } ?>
...
<li class="light-green">
    <a href="<?php  echo create_url('mobile/channel', array('name'=>index,'weid' => $_W['weid']))?>" target="_blank">
...
<li class="purple">
        ...
        <ul class="pull-right dropdown-navbar navbar-pink dropdown-menu dropdown-caret" style="height: 300px;overflow-y:scroll;">
            ...
            <?php  if($_W['isfounder']) { ?>
                <?php  if(is_array($_W['founder_wechats'])) { foreach($_W['founder_wechats'] as $account) { ?>
                <li>
...
<div class="sidebar-shortcuts" id="sidebar-shortcuts">
    <div class="sidebar-shortcuts-large" id="sidebar-shortcuts-large">
        <a title="粉丝统计" href="<?php  echo create_url('site/module',array('do'=>'display','name'=>'fans'))?>" class="btn btn-success">
            <i class="icon-signal"></i>
        </a>
        <a title="个人资料" href="<?php  echo create_url('setting/profile')?>" class="btn btn-info">
            <i class="icon-pencil"></i>
        </a>
        <a title="公众号列表" href="<?php  echo create_url('account/display',array('type'=>1))?>" class="btn btn-warning">
            <i class="icon-group"></i>
        </a>

        <a title="公众号管理" href="<?php  echo create_url('account/post',array('id'=>'current'))?>" class="btn btn-danger">
            <i class="icon-cogs"></i>
        </a>
    </div>
...
<ul class="nav nav-list">
    ...
    <ul class="submenu">
        <?php  $j=0;?>
        <?php  if(is_array($menu['items'])) { foreach($menu['items'] as $_item) { ?>
        <?php  $j++;?>
        <li menuorder="<?php  echo $j;?>" <?php  if($menuorder['0']==$i && $menuorder['1']==$j) { ?>class="active"<?php  } ?>>
        <a href="<?php  echo $_item['1'];?>" >
    ...
    <script type="text/javascript">
       $(function(){
           $('.nav-list').on('click','a', function(e){
               ...
               $.cookie( '<?php  echo $_W['config']['cookie']['pre'];?>'+'menuorder', getNodeIndex.call( this, '') , { expires : 0 });
           })
       })
    </script>
</ul><!-- /.nav-list -->
    ...
    <div class="main-content">
        <div class="breadcrumbs" id="breadcrumbs">
            ...
            <ul class="breadcrumb">
				...
				<?php  if(is_array($_W['breadcrumb'])) { foreach($_W['breadcrumb'] as $_key => $_item) { ?>
                	<?php  if($_key == (count($breadcrumb)-1)||empty($_item['url'])) { ?>
                	 <li class="active"><?php  echo $_item['title'];?></li>
                	 <?php  } else { ?>
	                <li>
        	            <a href="<?php  echo $_item['url'];?>"><?php  echo $_item['title'];?></a>
    	            </li>
                    <?php  } ?>
                <?php  } } ?>
                <?php  } ?>
