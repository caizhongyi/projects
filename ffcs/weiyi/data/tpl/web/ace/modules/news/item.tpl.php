...
<table id="form" class="tb reply-news-edit <?php if(!empty($news)) { ?>hide<?php } ?>">
	<tr>
		<th>素材<?php echo $_GPC['title'];?></th>
		<td>
			<button class="btn btn-sm btn-primary" onclick="showNewsForm('<?php echo $itemid;?>')" type="button">选择素材</button>
			<span class="help-block">素材是由系统其它功能提供的一些数据，例如：文章、相册、商户等</span>
		</td>
	</tr>
	<tr>
		<th>标题</th>
			...
			<div id="" class="uneditable-input reply-edit-cover">
				<div class="detail">
                    ...
                    <div  class="ace-file-input ">
                        <label data-title="上传" class="file-label selected"><span data-title="上传的文件" class="file-name"><i class="icon-file"></i></span></label>
                    </div>
                    ...
                    <button type="button" class="btn btn-upload-delete reply-news-edit-cover-remove" id="upload-delete" onclick="doDeleteItemImage(this, 'news-picture-value')" style="<?php if(empty($news['thumb'])) { ?> display:none;<?php } ?>"><i class="icon-remove"></i> 删除</button>
                </div>
