...
<?php } ?>
<div id="show" class="span6 alert alert-info clearfix <?php if(empty($item)) { ?>hide<?php } ?>" >
    <div class="content" id="content"><?php echo $item['content'];?></div>
    <span class="pull-right">
		<?php if(!empty($item)) { ?><a href="<?php echo create_url('index/module/delete', array('name' => 'basic', 'id' => $item['id']))?>" onclick="return doDeleteItem('<?php echo $itemid;?>', this.href)" style="margin-right:5px;">删除</a><?php } else { ?><a onclick="doDeleteItem('<?php echo $itemid;?>')" href="javascript:;" style="margin-right:5px;">删除</a><?php } ?>
...
<div id="form" class="alert alert-block alert-new <?php if(!empty($item)) { ?>hide<?php } ?>">
	...
	<table>
		...
		<tr>
			<th></th>
			<td><button type="button" onclick="basicHandler.doAdd('<?php echo $itemid;?>')" class="btn btn-sm btn-primary ">添加</button></td>
		</tr>
	</table>
</div>