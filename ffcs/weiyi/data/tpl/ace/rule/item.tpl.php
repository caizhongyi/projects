...
<div id="form" class="alert alert-block alert-new <?php if(!empty($item)) { ?>hide<?php } ?>">
	...
	<table>
			...
			<td>
				...
				<div class="btn-group" data-toggle="buttons">
					<?php if(is_array($types)) { foreach($types as $value => $type) { ?>
						...
						<?php } ?>
                        <label class="btn btn-sm btn-primary <?php if($value == $item['type']) { ?>active<?php } ?>" description="<?php echo $type['description'];?>" >
                            <input type="radio"  name="options" id="option1" value="<?php echo $value;?>" > <?php echo $type['name'];?>
                        </label>
					<?php } } ?>
				</div>
				<?php if(empty($item)) { ?>
		...
		<tr>
			<th></th>
			<td><button type="button" class="btn btn-sm btn-primary" onclick="keywordHandler.doAdd('<?php echo $itemid;?>')">添加</button></td>
		</tr>
	</table>
</div>
