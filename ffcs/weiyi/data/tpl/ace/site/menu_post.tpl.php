...
<div class="main">
	...
	<table class="tb">
		...
		<tr>
			...
			<td>
				<select name="position" class="position"  id="position" onchange="togglemenu($(this).val())" autocomplete="off">
					...
					<option value="<?php echo $posid;?>" <?php if($item['position'] == $posid) { ?> selected<?php } ?>><?php echo $posname;?></option>
					<?php } } ?>
				</select>
		...
		<tr>
			...
			<td>
				<div class="radio inline"><label for="type_1"><input type="radio" name="type" value="0" id="type_1" <?php if($item['issystem']==0 || empty($item)) { ?> checked<?php } ?> autocomplete="off" /> 系统内置菜单</label></div>
                <div class="radio inline"><label for="type_2"><input type="radio" name="type" value="1" id="type_2" <?php if($item['issystem']==1 && !empty($item)) { ?> checked<?php } ?> autocomplete="off" /> 自定义菜单</label></div>
                <div class="radio inline"><label for="type_3"><input type="radio" name="type" value="2" id="type_3" <?php if($item['issystem']==2 && !empty($item)) { ?> checked<?php } ?> autocomplete="off" /> 分类菜单</label></div>
                <span class="help-block"></span>
			</td>
		</tr>
		<tbody id="tbody_0" <?php if($item['issystem']!=0) { ?> style="display:none;"<?php } ?>>
		<tr>
			...
			<td>
				...
				<select name="sysurl" autocomplete="off">
					...
					<option value="<?php echo $menu['url'];?>" <?php if($item['url'] == $menu['url']) { ?> selected<?php } ?>><?php echo $menu['title'];?></option>
					<?php } } ?>
				</select>
		...
		<tbody id="tbody_1" <?php if($item['issystem']!=1) { ?> style="display:none;"<?php } ?>>
		<tr>
			<th><label for="">链接</label></th>
			<td>
				<div class="input-group" >
					<input class="span3" type="text" name="url" id="url" value="<?php echo $item['url'];?>" placeholder=""><button class="btn btn-sm btn-primary" onclick="w = $('#modal-module-menus').modal();buildModuleMenuForm();return false;">选择模块扩展链接</button>
				</div>
        ...
        <tbody id="tbody_2" <?php if($item['issystem']!=2) { ?> style="display:none;"<?php } ?>>
        <tr>
            <th><label for="">链接</label></th>
            <td>
                <div class="input-group">
                    <input class="span3" type="text" name="url" id="groupurl" value="<?php echo $item['url'];?>" placeholder=""><button class="btn btn-sm btn-primary" onclick="w = $('#modal-group-menus').modal();buildGroupMenuForm();return false;">选择分类链接</button>
                </div>
	...
	<table class="tb">
		<tr>
			...
			<td>
				<div class="input-group" >
					<input class="span3" type="text" name="icon[icon]" id="icon" value="<?php echo $item['css']['icon']['icon'];?>" placeholder=""><button class="btn btn-sm btn-primary" onclick="w = ajaxshow('<?php echo create_url('site/icon')?>', '图标列表', {width : 800});return false;">选择图标</button>
				</div>
		...
		<tr>
			...
			<td>
				<div class="fileupload fileupload-new" data-provides="fileupload">
					<div class="fileupload-preview thumbnail" ><?php if($item['fileicon']) { ?><img src="<?php echo $_W['attachurl'];?><?php echo $item['fileicon'];?>" width="50" /><?php } ?></div>
					<div>
...
<div id="modal-group-menus" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true" style="position:absolute; width:600px;">
    ...
    <div class="modal-body">
        <table class="tb">
            <tr>
                ...
                <td>
                    <div class="input-append" style="display:block;">
                        <input type="text" class="span3" name="keyword" value="" id="groupKeyword" /><button class="btn" onclick="buildGroupMenuForm(); return false;">搜索</button>
                    </div>
                </td>
            </tr>
        </table>
...
<!--
    ...
    function buildGroupMenuForm() {
        ...
        try {
            $.ajax({
                url: "<?php echo create_url('site/nav/group')?>",
                type: "GET",
