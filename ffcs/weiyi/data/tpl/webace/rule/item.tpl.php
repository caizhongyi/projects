...
<div id="form" class="alert alert-block alert-new <?php if(!empty($item)) { ?>hide<?php } ?>">
	...
	<table>
		...
		<tr>
			<th>对应方式</th>
			<td>
				<input type="hidden" id="keyword-type-new" name="keyword-type<?php echo $namesuffix;?>" class="span7" value="<?php echo $item['type'];?>" autocomplete="off">
				<div class="btn-group" data-toggle="buttons">
					<?php if(is_array($types)) { foreach($types as $value => $type) { ?>
						<?php if($value == $item['type']) { ?>
							<?php $currenttype = $type;?>
						<?php } ?>
                        <label class="btn btn-primary <?php if($value == $item['type']) { ?>active<?php } ?>" description="<?php echo $type['description'];?>" >
                            <input type="radio"  name="options" id="option1" value="<?php echo $value;?>" > <span><?php echo $type['name'];?></span>
                        </label>
					<?php } } ?>
				</div>
				<?php if(empty($item)) { ?>
		...
		<tr>
			<th></th>
			<td><button type="button" class="btn btn-primary btn-sm" onclick="keywordHandler.doAdd('<?php echo $itemid;?>')">添加</button></td>
		</tr>
	</table>
</div>
