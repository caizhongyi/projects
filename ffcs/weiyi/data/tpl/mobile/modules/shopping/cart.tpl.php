...
function priceTotal() {
	...
	$('.order-detail-list > .media').each(function() {
	    var price = parseFloat($(this).find('.price span').html()) ,count = parseInt($(this).find('.menu-list-num').html());
        a = a.add(price.mul(count));
	});
...
function checkform(form) {
 ...
 /*       $.post( window.location.href ,$('#formOrder').serializeArray() , function(data){
            console.log( data )
            location.href = "http://localhost/weiyi/mobile.php?act=module&orderid=20&name=shopping&do=pay&weid=46"
        })
    return false;*/
}
</script>
