 ...
 *  修改特殊类型回复如地理位置等回复  modify by hongmingjie 2014/02/03
 */
...
if ($do == 'display') {
	...
	//修改特殊类型回复如地理位置等回复  modify by hongmingjie 2014/02/03
        if (is_array($_W['account']['default_message'])) {
            $defaule_message_type = array('location','image','link','video','voice');
            foreach($defaule_message_type as $k => $v){
                if(is_array($_W['account']['default_message'][$v])){
                    $wechat[$v] = rule_single($_W['account']['default_message']['location']['id']);
                    $wechat[$v.'id'] = $_W['account']['default_message']['location']['id'];
                }
            }
	...
	if(checksubmit()) {
	//修改特殊类型回复如地理位置等回复  modify by hongmingjie 2014/03/03
		...
		foreach($s as $k => &$v) {
           ...
           if(is_array($df[$k])&&empty($v)){
                $v = $df[$k]['module'];
            }
		}
		$row = array();
		$row['default_message'] = iserializer($s);
		//更新bug,如无变更提示失败  modify by hongmingjie 2014/03/03
		if(pdo_update('wechats', $row, array('weid' => $_W['weid']))===false) {
            message('保存失败, 请稍后重试. ');
		} else {
	...
	//修改特殊类型回复如地理位置等回复  modify by hongmingjie 2014/03/03
        if(is_array($_W['account']['default_message'][$k])){
            $row['current'] = is_array($_W['account']['default_message']) ? $_W['account']['default_message'][$k]['module'] : '';
        }else{
	...
	}elseif ($_GPC['type'] == 'location' || $_GPC['type'] == 'image '|| $_GPC['type'] == 'video' || $_GPC['type'] == 'link '|| $_GPC['type'] == 'voice'|| $_GPC['type'] == 'image' ){
        //这边对地理位置、链接、音频、图片等做出响应。放在default_message这字段中。modify by hongmingjie 2014/03/03
        global $_W;
        $default_message = $_W['account']['default_message'];
        //type表示默认类型回复。
        $default_message[$_GPC['type']] = array(
            'module' => $rule['module'],
            'id' => $rid,
            'type'=>'default'
        );
        $value = iserializer($default_message);
        $data = array(
	...
	} elseif(in_array(trim($_GPC['type']),$defaule_message_type)){
        global $_W;
        $default_message = $_W['account']['default_message'];
        //type表示默认类型回复。
        $default_message[$_GPC['type']] = '';
        $value = iserializer($default_message);
        $data = array(
            'default_message'=>$value,
        );
