...
if (checksubmit('submit')) {
	...
	if (! $_W['isfounder']) {
		$maxaccount = pdo_fetchcolumn("SELECT maxaccount FROM " . tablename('members_group') . " WHERE id = :id", array(
			':id' => $_W['member']['groupid']
		));
		if (! empty($maxaccount)) {
			$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('wechats') . " WHERE uid = :uid", array(
				':uid' => $_W['uid']
			));
	...
	if (!empty($id)) {
		...
		pdo_update('wechats', $update, array(
			'weid' => $id
		));
	...
	isetcookie('__weid', $newid, 7 * 46800);
