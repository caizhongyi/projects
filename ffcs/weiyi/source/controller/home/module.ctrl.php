...
if(!$_W['isajax'] && !$_W['ispost'] && !isset($_GPC['force'])) {
	message("模块操作入口推荐使用 site.php, 当前还可以继续访问, 但是推荐更新你的代码, 微翼系统将在下一个版本不支持这种操作调用方式. <hr />请修改你的模块以便使用新的操作入口机制(将你定义在 module.php 里的操作 do{$_GPC['do']} 移动至 site.php 并命名为 doWeb{$_GPC['do']}. 并使用 create_url('site/module/{$_GPC['do']}', array('name' => '{$_GPC['name']}', 'state' => '{$_GPC['state']}')) 来获得新的访问入口地址) . 如果你不能理解这里的内容, 请将这个提示信息提供给你的模块开发者.", '?' . $_SERVER['QUERY_STRING'] . '&force=1');
}
