...
function account_search($uid = 0, $type = 0) {
    ...
    $arr_uid = get_son_uids($uid);
    $condition .= $_W['isfounder'] ? '' : " AND uid in (".implode(',',$arr_uid).")";
...
渠道商账号与子账号的公众号
 */
function account_channel_search($uid = 0, $type = 0){
    global $_W;
    $uid = empty($uid) ? $_W['uid'] : $uid;
    if($_W['member']['groupid'] == CHANNEL_GROUP_ID){
        $count = pdo_fetchcolumn("select count(*) from ".tablename('members_channel')." where parentid = '{$uid}'");
        if($count > 0){
            $condition = "where 1=1";
            $condition .= empty($type) ? '' : " AND b.type = '{$type}'";
            $condition .= " AND (b.uid = '{$uid}' or (a.childid = b.uid AND a.parentid = '{$uid}') )";
            $sql = "select * from ".tablename('members_channel')." as a, ".tablename('wechats')." as b $condition ORDER BY b.weid DESC";
            $result = pdo_fetchall($sql, array(), 'weid');
        }else{
...
/*
 * 添加刷新token参数 modify by hongmingjie 2014/03/04
 */
function account_weixin_token($account,$refresh=false) {
	if(!$refresh && is_array($account['access_token']) && !empty($account['access_token']['token']) && !empty($account['access_token']['expire']) && $account['access_token']['expire'] > TIMESTAMP) {
...
/*
 * 增加一个refresh参数，直接使用刷新后的token
 */
function account_yixin_token($account,$refresh=false) {
	if(!$refresh&&is_array($account['access_token']) && !empty($account['access_token']['token']) && !empty($account['access_token']['expire']) && $account['access_token']['expire'] > TIMESTAMP) {
		return $account['access_token']['token'];
	} else {
		if(empty($account['weid'])) {
...
function account_yixin_basic($username) {
    ...
    //获取appid，appscrappsecret add by hongmingjie 2014/02/18
    $response = ihttp_request(YIXIN_ROOT . '/advanced/developer', '', array('CURLOPT_COOKIE' => $auth['cookie']));
    preg_match('/AppID<\/label><span class=\"u-ipt\">(.*?)<\/span>/', $response['content'], $match);
    $info['key'] = $match[1];
    preg_match('/AppSecret<\/label><span class=\"u-ipt\">(.*?)<\/span>/', $response['content'], $match);
    $info['secret'] = $match[1];
	return $info;
}
