...
class WxwallModuleProcessor extends WeModuleProcessor {
	...
	private function register() {
		...
		if($wall == false || (!empty($wall['end_time']) && $wall['end_time'] < time())){
			$message = '活动已经取消或结束了';
			return $this->respText($message);
		}
		if(!empty($wall['start_time']) && $wall['start_time'] > time()){
	...
	private function post() {
		...
		if ($this->message['type'] == 'image') {
			...
			require IA_ROOT . '/source/modules/wxwall/thumb.php';
			$thumb_obj = new thumb();
			$thumb_img = $thumb_obj->image2thumb(IA_ROOT.'/resource/attachment/', $filename, '', '100', '100');
			$data['thumb'] = $thumb_img;
		}
