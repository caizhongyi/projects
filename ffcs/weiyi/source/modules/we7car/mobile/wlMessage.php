...
if($op=='ajax'){
    ...
    $totalpage = ($messagecount%$pagenum==0) ? ($messagecount/$pagenum) : (floor($messagecount/$pagenum)+1);
	$prow=($p-1)*$pagenum;
    ...
    //分页信息
   if($totalpage>$p){
        $nextpage=$this->createMobileUrl('message', array('id' => $_GPC['id'], 'op'=>'list', 'p'=>($p+1)));
    }else{
        $nextpage=$this->createMobileUrl('message', array('id' => $_GPC['id'], 'op'=>'list', 'p'=>$totalpage));
    }
    if($p>1){
        $prepage=$this->createMobileUrl('message', array('id' => $_GPC['id'], 'op'=>'list', 'p'=>($p-1)));
    }else{
        $prepage=$this->createMobileUrl('message', array('id' => $_GPC['id'], 'op'=>'list', 'p'=>1));
    }
