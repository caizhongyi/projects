...
class EggModuleSite extends WeModuleSite {
	...
	public function doMobileLottery() {
		...
		$profile = fans_require($fromuser, array('realname', 'mobile'), '需要完善资料后才能砸蛋.');
        $id = intval($_GPC['id']);
	...
	public function doMobileGetAward() {
		...
		$fromuser = $_W['fans']['from_user'];
        $id = intval($_GPC['id']);
        $egg = pdo_fetch("SELECT id, periodlottery, maxlottery, default_tips, misscredit, hitcredit, start_time, end_time FROM ".tablename('egg_reply')." WHERE rid = '$id' LIMIT 1");
        if (empty($egg)) {
			message('非法访问，请重新发送消息进入砸蛋页面！');
        	}
		$result = array('status' => -1, 'message' => '');
		//判断时间是否过期     
    	if(($egg['start_time']<time() && $egg['end_time']>time()) || (empty($egg['start_time']) && empty($egg['end_time'])) ){
    		$is_expired = false;//没过期
    	}else{
    		$is_expired = true;//过期
			$result = array('status'=>-2,'message'=>'砸蛋活动已经结束！');
    		message($result, '', 'ajax');
    		die();
    	}
