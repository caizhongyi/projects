	...
	private function formatMsg(&$list) {
		...
		foreach ($list as &$row) {
			...
			if ($row['type'] == 'link') {
				...
				$row['content'] = '<a href="'.$_W['attachurl'] . $row['content'].'" style=""><img src="'.$_W['attachurl'] . $row['thumb'].'" style="width:100px;"/>';
			//	$row['content'] = '<a href="'.$_W['attachurl'] . $row['content'].'" style=""><img src="'.$_W['attachurl'] . $row['content'].'" style="width:100px;" />';
