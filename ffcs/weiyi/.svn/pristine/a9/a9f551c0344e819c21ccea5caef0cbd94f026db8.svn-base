...
if($op=='post'){
	...
	if ($_GPC['submit']!=''){
		...
		if($temp==false){
			message('不需要执行更新操作',referer(), 'error');
		}else{
    ...
    if (isset($_GPC['type'])){
        $condition .= " AND yytype = {$_GPC['type']} ";
    }
	$list = pdo_fetchall("select * from ".tablename('weicar_yuyue_list')." where weid=".$weid .$condition."  order by id desc LIMIT ".($pindex - 1) * $psize.','.$psize);
    ...
    $id=$_GPC['id'];
    $weid =$_GPC['weid'];
    $row = pdo_fetch("SELECT id FROM ".tablename('weicar_yuyue_set')." WHERE id = :id and weid = :weid", array(':id' => $id, ':weid' => $weid));
    if (empty($row)) {
        message('抱歉，该知预约项不存在或者已经删除！');
    }
    pdo_delete('weicar_yuyue_set', array('id' => $id, 'weid' => $weid));
