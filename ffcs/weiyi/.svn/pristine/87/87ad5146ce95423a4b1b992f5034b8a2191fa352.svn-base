    ...
    wgt.hotMobile = {
        ...
        addOrder: function(isAddCart){
        	...
        	if(!isAddCart){
	        	...
	        	var ofrs = $("#hyjh_list input:checked").val().split(",");
	        	orderInfo.contractId = ofrs[2];
	        	orderInfo.contractName = $("#heiyue_name span").html();
	        	orderInfo.ofrId = ofrs[3];
	        	orderInfo.selNum = $("#number_list [name='number']:checked").val();
        	}
            if(!self.cntPhone.distributionName){
                $.error_pop("温馨提示","对不起，此款商品无库存");
                return;
            }
            var price = self.cntPhone.price;
        	orderInfo.prodId = self.searchData.id;
        	orderInfo.prodType = "1";
        	orderInfo.prodName = self.cntPhone.name + " " + self.cntPhone.color;
        	orderInfo.distributionName = self.cntPhone.distributionName;
        	orderInfo.prodImage = self.cntPhone.imageUrl;
        	orderInfo.proColor = self.cntPhone.color;
        	orderInfo.customize = self.cntPhone.customize;
        	orderInfo.priceRemark = $.trim(self.$numberPrice.html());
        	orderInfo.detailId = self.cntPhone.detailId;
        	orderInfo.prodPrice = price;
        	orderInfo.appendPrice = self.getPrestorePrice();
        	orderInfo.count = 1;
        	orderInfo.totalPrice = parseFloat(price);
        	//objUtil.showAttributes(orderInfo);
        	// 参入后台的参数对象
    		var params = new Object();
    		params.orderInfoStr = $.toJSON(orderInfo).replace(/\+/g, "%2B");
			if(isAddCart){
				params.isAddCart = "1";
			}
        	self.ajaxPOST('/hotModel/hotModelManage!saveMakeOrder.action', params, function(json){
            	if (json.success) {
            		if(isAddCart){
			            $.slideDownMsg("添加购物车成功.");
            		}else{
	            		window.location.href = common.getWebRoot() + '/hotModel/hotModelManage!initMakeOrder.action';
            		}
            	} else {
        ...
        getNumber: function(){
            ...
            self.ajaxGetNumber = self.ajaxPOST ('/hotModel/hotModelManage!searchNum.action', self.searchData, function(json){
