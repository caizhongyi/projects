...
class ResearchModuleSite extends WeModuleSite {
	...
	public function doWebPost() {
		...
		if(checksubmit()) {
			...
			$recrod['weid'] = $_W['weid'];
			//$recrod['description'] = trim($_GPC['description']);htmlspecialchars_decode
            $recrod['description'] = trim($_GPC['description']);
            $recrod['description'] = htmlspecialchars_decode($recrod['description']);
            ...
            if($recrod['starttime']>=$recrod['endtime'])
            {
                message('对不起，预约开始时间不能大于开始时间。');
            }
			if(empty($reid)) {
				$recrod['status'] = 1;
