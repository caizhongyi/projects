...
class WeEngine {
	...
	public function start() {
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            ...
            if(!($this->params['module']=='default' && $this->response['MsgType']=="text" && empty($this->response['Content']))){
    ...
    /*
     * 基于地理位置透传功能,modify by hongmingjie 2014/03/03
     */
	private function matcherLocation() {
		$params = array();
		global $_W;
		$df = $_W['account']['default_message'];
		if(is_array($df) && !empty($df['location']['module']) && in_array($df['location']['module'], $this->modules)) {
			$params[] = array('module' => $df['location']['module'], 'rule' => $df['location']['id']);
		}elseif(is_array($df) && in_array($df['location'], $this->modules)) {
	...
	public static function parse($message) {
		...
		if (!empty($message)){
			...
			if($obj instanceof SimpleXMLElement) {
                ...
                //发送内容
                $packet['content'] = trim($packet['content']);
...
abstract class WeModuleSite {
	...
	protected function template($filename, $flag = TEMPLATE_INCLUDEPATH) {
		...
		if($this->inMobile) {
			...
			//优先使用系统指定的模板 by fangyq 2014-1-22
			if(!is_file($source)) {
				$source = "{$_W['template']['source']}/web/{$_W['template']['current']}/{$filename}.html";
				$compile = "{$_W['template']['compile']}/web/{$_W['template']['current']}/{$filename}.tpl.php";
			}
