...
/*2.0soap地址*/
$soap_url = "http://42.121.125.119/vieasy/BnetForSIWS.wsdl";
//$soap_url = "http://bnetforap.fj.bnet.cn/AsyncInterfaceForSI/v1_2/BnetForSIWS.asmx?wsdl";
...
if($decode == $encode){		//验证通过，处理平台业务
	...
	if($opflag == '0101'){		//订购注册
		...
		if(is_array($member)){
			...
			isetcookie('__session', $session, 0);
