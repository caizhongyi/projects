...
class Yixin {
    ...
    public function __construct($options){
        ...
        $this->weid = isset($options['weid'])?$options['weid']:'';
        $this->account = $options;
        if(is_array($options['access_token']) && !empty($options['access_token']['token']) && !empty($options['access_token']['expire']) && $options['access_token']['expire'] > TIMESTAMP) {
            $this->access_token =  $options['access_token']['token'];
        }else{
            $this->access_token = '';
        }
    }
...
/*    public function get_access_token() {
    ...
    function get_access_token($refresh=false) {
        require_once IA_ROOT . '/source/model/account.mod.php';
        $this->access_token = account_yixin_token( $this->account,$refresh);
    }
    ...
    public function send($openid,$content,$type="text"){
        ...
        $content = $this->sendPost($this->host,$url,$data,true,1);
