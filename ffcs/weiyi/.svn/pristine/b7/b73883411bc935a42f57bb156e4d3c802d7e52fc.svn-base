...
function tpl_form_field_color2($name, $value = '') {
    $s = '';
    if (!defined('INCLUDE_KINDEDITOR')) {
        $s = '
		<script type="text/javascript" src="./resource/script/colorpicker/spectrum.js"></script>
...
function tpl_form_field_image($name, $value = '', $default = '') {
    ...
    $url = $GLOBALS['_W']['attachurl'] . $value;
	$s .= '
	...
	<button class="btn btn-info" type="button" id="upload-image-'.$id.'">选择图片</button>
	</div>
	<div id="upload-image-preview-'.$id.'" style="margin-top:10px;">';
    if (!empty($value)){
        $s .= '<img src="'. $url.'" width="100" />';
    }elseif (!empty($default)){
        $s .= '<img src="'. $default.'" width="100" />';
    }else{
    }
    $s .= '</div>
	<script type="text/javascript">
