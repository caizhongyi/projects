...
if($_W['ispost']) {
    ...
    $bestpay = array_elements(array('switch', 'merchantid','bestpay_key'), $_GPC['bestpay']);
    $bestpay['switch'] = $bestpay['switch'] == 'true';
    $bestpay['merchantid'] = trim($bestpay['merchantid']);
    $bestpay['bestpay_key'] = trim($bestpay['bestpay_key']);
    if($bestpay['switch'] && (empty($bestpay['merchantid'])) && (empty($bestpay['bestpay_key']))) {
        message('请输入完整的翼支付接口信息.');
    }

    $ondelivery = array_elements(array('switch'), $_GPC['ondelivery']);
    $ondelivery['switch'] =  $ondelivery['switch'] == 'true';
	...
	$payment['bestpay'] = $bestpay;
	$payment['ondelivery'] = $ondelivery;
...
$ondelivery  = $_W['account']['payment']['ondelivery'];
