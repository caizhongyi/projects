...
function mobile_nav($position) {
	global $_W;
	$navs = pdo_fetchall("SELECT name, url, icon, css, position, module FROM ".tablename('site_nav')." WHERE position = '$position' AND status = 1 AND weid = '{$_W['weid']}' ORDER BY displayorder DESC");
	if (!empty($navs)) {
