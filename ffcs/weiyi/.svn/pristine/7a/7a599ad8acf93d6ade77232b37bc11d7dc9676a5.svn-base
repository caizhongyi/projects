...
function message(msg, redirect, type) {
	...
	function _message(msg, redirect, type) {
		...
		if(modalobj.length == 0) {
			$(document.body).append('<div id="modal-message" class="modal  fade" tabindex="-1" role="dialog"  aria-labelledby="myModalLabel" ><div class="modal-dialog"><div class="modal-content"></div></div></div>');
			var modalobj = $('#modal-message');
		}
		...
		html = '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3 id="myModalLabel">系统提示</h3></div>' +
				'<div class="modal-body clearfix"><i class="icon-ok icon-large icon-3x pull-left"></i><div class="pull-left modal-message"><p>'+ msg +'</p>' +
...
function ajaxshow(url, title, options, events) {
	var modalobj = $('#modal-message');
    modalobj.removeClass('alert-error alert alert-success');//去除错误样式
	...
	if(modalobj.length == 0) {
		$(document.body).append('<div id="modal-message" class="modal  fade" tabindex="-1" role="dialog" aria-hidden="true"  aria-labelledby="myModalLabel" ><div class="modal-dialog"><div class="modal-content"></div></div></div>');
		var modalobj = $('#modal-message');
	}
...
function doEditItem(itemid) {
	$('#append-list .item').each(function(){
        ...
        $('#show', $(this)).removeClass('hide').css('display','block');
	});
	var parent = $('#' + itemid);
 ...
 * @returns {Boolean}
 */
function doDeleteItemImage(obj, name,callback) {
    var that = obj;
    var upload_input =  $(obj).parent().parent().find('input[name=' + name+']');
	...
	ajaxopen('./index.php?act=attachment&do=delete&filename=' + filename, function(){
		...
		upload_input.val('');
        //ace 版上传控件 与原上传控件 同步
        $(that).closest('.detail').find('.file-name').attr('data-title','上传文件');
        callback && callback.deleteSuccess && callback.deleteSuccess(obj)//删除成功回调
	});
	return false;
}
		...
		var uploadbutton = KindEditor.uploadbutton({
			...
			afterUpload : function(data) {
				if (data.error === 0) {
					...
					$(uploadbutton.div.parent().parent()[0]).find('#upload-file-view').html('<input value="'+data.filename+'" type="hidden" name="'+obj.attr('fieldname')+'" id="'+obj.attr('id')+'-value" /><img id="'+obj.attr('id')+'-view" src="'+url+'" width="100" />');
                    ...
                    if(callback){
                        if(typeof callback == 'object'){
                            callback.uploadSuccess && callback.uploadSuccess.call( this ,obj ,data.url)//上传成功回调
                        }
                        else{
                            callback.call( this ,obj ,data)//上传成功回调
                        }
                    }
				} else {
...
function selectall(obj, name){
    $(obj).closest('table').prev().find(':checkbox').prop('checked', $(obj).prop('checked'));
}
...
//ace 版上传控件 与原上传控件 同步
function aceUploadFileAnsy( elem ){
    var val = $(elem).val();
    $(elem).closest('.detail').find('.file-name').attr('data-title', val );
 //   $(elem).closest('.ke-inline-block').prev().val( val )
}
/*$(window).on('load',function(){ console.log(1)
    $('.ke-upload-file').on('change',function(){
    console.log(this)
