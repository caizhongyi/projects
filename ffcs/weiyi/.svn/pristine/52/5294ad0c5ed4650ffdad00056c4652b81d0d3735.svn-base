...
function message($msg, $redirect = '', $type = '') {
	...
	elseif (defined('IN_MANAGEMENT')) {//后台消息提示
...
function checkpermission($type, $target) {
	...
	if ($type == 'wechats') {
		if (is_array($target)) {
			...
			$account = pdo_fetch("SELECT uid, weid FROM ".tablename('wechats')." WHERE weid = '$target' LIMIT 1");
		}

        $uid = $_W['uid'];
        $arr_uid = get_son_uids($uid);

		if (!in_array($account['uid'],$arr_uid)) {
			return false;
		}
	}
	return true;
 ...
 */
function pagination($tcount, $pindex, $psize = 15, $url = '', $context = array('before' => 5, 'after' => 4, 'ajaxcallback' => ''),$pagename="page") {
	global $_W;
	$pdata = array(
		'tcount' => 0,
		'tpage' => 0,
	...
	if($context['isajax']) {
		...
		if($url) {
			...
			$_GET[$pagename] = $pdata['findex'];
			$pdata['faa'] = 'href="' . $_W['script_name'] . '?' . http_build_query($_GET) . '"';
			$_GET[$pagename] = $pdata['pindex'];
			$pdata['paa'] = 'href="' . $_W['script_name'] . '?' . http_build_query($_GET) . '"';
			$_GET[$pagename] = $pdata['nindex'];
			$pdata['naa'] = 'href="' . $_W['script_name'] . '?' . http_build_query($_GET) . '"';
			$_GET[$pagename] = $pdata['lindex'];
			$pdata['laa'] = 'href="' . $_W['script_name'] . '?' . http_build_query($_GET) . '"';
		}
	}

	$html = '<div style="text-align: center;" ><ul class="pagination " >';
	if($pdata['cindex'] > 1) {
...
function create_url($router, $params = array()) {
	...
	return $module . '.php?act=' . $controller . (empty($do) ? '' : '&do=' . $do) . (empty($queryString) ? '' : '&' . $queryString);
}
/**
 * 获取引用页
 */
 ...
 * @param int $uid
 */
function get_son_uids($uid) {
    $arr_uid = array($uid);
    $son_members_groups = pdo_fetchall("SELECT id FROM " . tablename('members_group') . " WHERE parent_ids='".$uid."'");
    foreach($son_members_groups as $son_members_group){
        $arr_tmp = pdo_fetchall("SELECT uid FROM " . tablename('members') . " WHERE groupid='".$son_members_group['id']."'");
        foreach($arr_tmp as $tmp){
            $arr_uid = array_merge($arr_uid,get_son_uids($tmp['uid']));
        }
    }
    return $arr_uid;
}
 ...
 * @param int $uid
 */
function get_son_groups($uid) {
    $arr_group = array();
    $son_members_groups = pdo_fetchall("SELECT id,name FROM " . tablename('members_group') . " WHERE parent_ids='".$uid."'");
    foreach($son_members_groups as $son_members_group){
        $arr_group[] = $son_members_group;
        $arr_tmp = pdo_fetchall("SELECT uid FROM " . tablename('members') . " WHERE groupid='".$son_members_group['id']."'");
        foreach($arr_tmp as $tmp){
            $arr_group = array_merge($arr_group,get_son_groups($tmp['uid']));
        }
    }
    return $arr_group;
}