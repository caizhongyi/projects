...
!(function (window) {
    ...
    // 将YixinApi暴露到window下：全局可使用，对旧版本向下兼容
    window.YixinApi = YixinApi;
    ...
    var _share = function (cmd, data, callbacks) {
        ...
        var handler = function (theData, argv) {
            ...
            if (argv && (argv.shareTo == 'favorite' || argv.scene == 'favorite')) {
                if (callbacks.favorite === false) {
                    YixinJSBridge.invoke('sendAppMessage', theData, new Function());
                } else {
                    YixinJSBridge.invoke(cmd.action, theData, progress);
                }
            } else {
                // 新的分享接口，单独处理
                if (cmd.menu === 'general:share') {
                    if (argv.shareTo === 'timeline') {
                        YixinJSBridge.invoke('shareTimeline', theData, progress);
                    } else if (argv.shareTo === 'friend') {
                        YixinJSBridge.invoke('sendAppMessage', theData, progress);
                    } else if (argv.shareTo === 'QQ') {
                        YixinJSBridge.invoke('shareQQ', theData, progress);
                    } else if (argv.shareTo === 'weibo') {
                        YixinJSBridge.invoke('shareWeibo', theData, progress);
                    }
                } else {
                    YixinJSBridge.invoke(cmd.action, theData, progress);
                }
        ...
        // 监听分享操作
        YixinJSBridge.on(cmd.menu, function (argv) {
            callbacks.dataLoaded = callbacks.dataLoaded || new Function();
            if (callbacks.async && callbacks.ready) {
                YixinApi["_wx_loadedCb_"] = callbacks.dataLoaded;
                if (YixinApi["_wx_loadedCb_"].toString().indexOf("_wx_loadedCb_") > 0) {
                    YixinApi["_wx_loadedCb_"] = new Function();
                }
                callbacks.dataLoaded = function (newData) {
                    callbacks.__cbkCalled = true;
                    var theData = _extend(data, newData);
                    theData.img_url = theData.imgUrl || theData.img_url;
                    delete theData.imgUrl;
                    YixinApi["_wx_loadedCb_"](theData);
                    handler(theData, argv);
                };
                // 然后就绪
                if (!(argv && (argv.shareTo == 'favorite' || argv.scene == 'favorite') && callbacks.favorite === false)) {
     ...
     * @p-config    {Function}  all(resp)       无论成功失败都会执行的回调
     */
    YixinApi.shareToTimeline = function (data, callbacks) {
        _share({
            menu: 'menu:share:timeline',
            action: 'shareTimeline'
        }, {
     ...
     * @p-config    {Function}  all(resp)       无论成功失败都会执行的回调
     */
    YixinApi.shareToFriend = function (data, callbacks) {
        _share({
            menu: 'menu:share:appmessage',
            action: 'sendAppMessage'
        }, {
     ...
     * @p-config    {Function}  all(resp)       无论成功失败都会执行的回调
     */
    YixinApi.shareToWeibo = function (data, callbacks) {
        _share({
            menu: 'menu:share:weibo',
            action: 'shareWeibo'
        }, {
     ...
     * @p-config    {Function}  all(resp)       无论成功失败都会执行的回调
     */
    YixinApi.generalShare = function (data, callbacks) {
        _share({
            menu: 'general:share'
        }, {
            "appid": data.appId ? data.appId : '',
     ...
     * @param callback
     */
    YixinApi.disabledShare = function (callback) {
        callback = callback || function () {
            alert('当前页面禁止分享！');
        };
        ['menu:share:timeline', 'menu:share:appmessage', 'menu:share:qq',
            'menu:share:weibo', 'general:share'].forEach(function (menu) {
                YixinJSBridge.on(menu, function () {
                    callback();
                    return false;
                });
            });
     ...
     * @param {Array} srcList 图片地址列表
     */
    YixinApi.imagePreview = function (curSrc, srcList) {
        if (!curSrc || !srcList || srcList.length == 0) {
            return;
        }
        YixinJSBridge.invoke('imagePreview', {
            'current': curSrc,
            'urls': srcList
        });
    };
     ...
     * 显示网页右上角的按钮
     */
    YixinApi.showOptionMenu = function () {
     ...
     * 隐藏网页右上角的按钮
     */
    YixinApi.hideOptionMenu = function () {
     ...
     * 显示底部工具栏
     */
    YixinApi.showToolbar = function () {
        YixinJSBridge.call('showToolbar');
    };

    /**
     * 隐藏底部工具栏
     */
    YixinApi.hideToolbar = function () {
     ...
     *
     * 使用方法：
     * YixinApi.getNetworkType(function(networkType){
     *
     * });
     *
     * @param callback
     */
    YixinApi.getNetworkType = function (callback) {
        if (callback && typeof callback == 'function') {
            YixinJSBridge.invoke('getNetworkType', {}, function (e) {
                // 在这里拿到e.err_msg，这里面就包含了所有的网络类型
                callback(e.err_msg);
            });
        }
     ...
     * @p-config    {Function}  success(resp)   成功
     */
    YixinApi.closeWindow = function (callbacks) {
        callbacks = callbacks || {};
        YixinJSBridge.invoke("closeWindow", {}, function (resp) {
            switch (resp.err_msg) {
                // 关闭成功
                case 'close_window:ok':
                    callbacks.success && callbacks.success(resp);
    ...
    /**
     * 当页面加载完毕后执行，使用方法：
     * YixinApi.ready(function(Api){
     *     // 从这里只用Api即是YixinApi
     * });
     * @param readyCallback
     */
    YixinApi.ready = function (readyCallback) {
        ...
        var _hook = function () {
            var _YixinJSBridge = {};
            Object.keys(YixinJSBridge).forEach(function (key) {
                _YixinJSBridge[key] = YixinJSBridge[key];
            });
            Object.keys(YixinJSBridge).forEach(function (key) {
                if (typeof YixinJSBridge[key] === 'function') {
                    YixinJSBridge[key] = function () {
                        try {
                            var args = arguments.length > 0 ? arguments[0] : {},
                                runOn3rd_apis = args.__params ? args.__params.__runOn3rd_apis || [] : [];
                            ['menu:share:timeline', 'menu:share:appmessage', 'menu:share:weibo',
                        ...
                        return _YixinJSBridge[key].apply(YixinJSBridge, arguments);
                    };
                }
            });
        };
        ...
        if (readyCallback && typeof readyCallback == 'function') {
            ...
            if (typeof window.YixinJSBridge == "undefined") {
                if (document.addEventListener) {
                    document.addEventListener('YixinJSBridgeReady', wxReadyFunc, false);
                } else if (document.attachEvent) {
                    document.attachEvent('YixinJSBridgeReady', wxReadyFunc);
                    document.attachEvent('onYixinJSBridgeReady', wxReadyFunc);
                }
            } else {
                wxReadyFunc();
            }
     ...
     * @p-config    {Function}  all(resp)       无论成功失败都会执行的回调
     */
    YixinApi.sendEmail = function (data, callbacks) {
        callbacks = callbacks || {};
        YixinJSBridge.invoke("sendEmail", {
            "title": data.subject,
            "content": data.body
        }, function (resp) {
            if (resp.err_msg === 'send_email:sent') {
     ...
     * @param    {Function}  callback(error) 出错后的回调，默认是alert
     */
    YixinApi.enableDebugMode = function (callback) {
        /**
         * @param {String}  errorMessage   错误信息
         * @param {String}  scriptURI      出错的文件
         * @param {Long}    lineNumber     出错代码的行号
...
//                alert(msgs.join(''));
     ...
     * @param wxCallbacks
     */
    YixinApi.share = function (wxData, wxCallbacks) {
        YixinApi.ready(function (Api) {
            // 用户点开右上角popup菜单后，点击分享给好友，会执行下面这个代码
            Api.shareToFriend(wxData, wxCallbacks);
...
!(function(window){
    ...
    var  share  = function( wxData, wxCallbacks ){
        window.YixinJSBridge && YixinApi.share( wxData, wxCallbacks );
        window.WeixinJSBridge && WeixinApi.share( wxData, wxCallbacks );
    };
