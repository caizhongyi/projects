    ...
    $.fn.tab = function(options) {
		...
		var el = $(this).each(function(){
				...
				var _init =  function()
				{
					...
					_this.inner.parent().css({"position":"relative",width:_this.inner.width(),height:_this.inner.height()});
					...
					case "fade":
						 ...
						 break;
					case "horizontal": 	_this.inner.css({'position':'absolute',left:0,top:0,width: innerWidth * _this.count});
						_this.inner.parent().css({'overflow':'hidden'});
					    innerChildren.css({"float":"left"});
						break;
					case "vertical": 
						_this.inner.css({'position':'absolute',left:0,top:0});
						_this.inner.parent().css({'overflow':'hidden'});
						break;
					default: break;
					};
