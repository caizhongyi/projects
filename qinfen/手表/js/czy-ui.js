...
;(function($) {
	$.fn.accordion = function(options)
	{
		...
		var item = $this.find(':[data-el=item]');
		this.index = 0;
		if(item.length == 0)
		{
			item  = this;
	    }
		var el = item.each(function(){
			var _this = this;
            var $this = $(this);
	        this.el = $this;
		    this.status=options.status;
		...
		return el.length <= 1 ? elements[0]: el;
	};
		/*
		@author 	: czy
		@description: 下拉框效果
    ...
    $.fn.tab = function(options) {
		...
		var el = $(this).each(function(){
				...
				var _init =  function()
				{
					...
					var innerWidth = _this.inner.children().width();
					var innerHeight = _this.inner.children().height();
					_this.inner.parent().css({"position":"relative",width:_this.inner.width(),height:_this.inner.height(),'overflow':'hidden'});
					...
					switch (options.animateType) {
					case "fade":
						 _this.inner.css({width: innerWidth,height:innerHeight});
						 innerChildren.css({"position":"absolute",left:0,top:0});  
						 break;
					case "horizontal": 	_this.inner.css({'position':'absolute',left:0,top:0,width: innerWidth * _this.count,height:innerHeight});
				...
				{
					obj.stop().animate({"left":-(obj.children().width() * _this.index)},options.animateSpeed, options.animateEasing,function(){if(options.auto){ createTimeOut();}});
				}
				
				var vertical = function(obj)
				{
					obj.stop().animate({"top":-(obj.children().height() * _this.index)},options.animateSpeed, options.animateEasing,function(){if(options.auto){ createTimeOut();}});
				}
    ...
    $.fn.imgView = function (options) {
        ...
        var el = this.each(function(i,n){
			...
			var _init = function()
			{
				...
				imgList.css({'position':'absolute',left:0,top:0,width:_this.count * imgList.children().width()});
				...
				imagesWidth = imgList.children().width() * _this.count;
			}
             
			this.move = function(val)
			{
				imgList.animate({left:val},options.animateSpeed,options.animateEasing);
			}
			this.prev = function()
			{
				var val = '-='+(moveVal);
				
				if(parseInt(imgList.css('left').replace('px','')) - (moveVal * 2) >= -imagesWidth )
				{
					_this.move(val);
				}
			}
			this.next = function()
			{
				...
				var val = '+='+(moveVal);
				if(parseInt(imgList.css('left').replace('px','')) + moveVal <= 0)
				{
					_this.move(val);
				}
			}
