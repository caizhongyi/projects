		...
		var el = this.each(function(){
			...
			var item =  function(el,index)
			{
				...
				var _init = function()
				{
					...
					{
						inner.height(_item.minHeight).css({'opacity' : 0 , 'overflow' : 'hidden'});
					}
				...
				this.hide = function()
				{
					...
					inner.stop();
					inner.animate({ height: _item.minHeight , opacity : 0},_item.speed ,_item.easing,
						function(){
							_item.status="hide";
    ...
    $.fn.tab = function(options) {
		...
		var el = $(this).each(function(){
				...
				var _init =  function()
				{
					...
					_this.children().width(innerWidth);
					_this.children().height(innerHeight);
